...
import com.alert.util.i18n.ButtonCaptionFactory;
import com.alert.util.icons.ButtonIconFactory;
import com.alert.util.icons.ClassicButtonIconFactory;
import com.alert.util.icons.ClassicDialogIconFactory;
import com.alert.util.icons.DialogIconFactory;
...
public class MessageBox implements Serializable {
	 ...
	 */
	protected static String DIALOG_DEFAULT_ICON_SIZE = "30px";

	/**
	 * Keeps the button default alignment
	 */
	protected static Alignment BUTTON_DEFAULT_ALIGNMENT = Alignment.BOTTOM_CENTER;
	 ...
	 */
	protected static boolean BUTTON_DEFAULT_ICONS_VISIBLE = false;
	...
	protected MessageBox() {
		...
		//contentLayout.setIcon(new ThemeResource("images/alert.png"));
	...
	protected MessageBox(Boolean isCritical,String info) {
		// Create window
		window = new Window();
		window.setClosable(false);
		window.setModal(true);
		...
		contentLayout.setMargin(false);
		contentLayout.setSpacing(true);
//		ThemeResource imgIcon = new ThemeResource("");

		if(info.equalsIgnoreCase(SHAConstants.ERROR_MSG_ICON)){
			ThemeResource imgIcon = new ThemeResource("images/error.png");
			Image image = new Image(null, imgIcon);
			contentLayout.addComponent(image);
			contentLayout.setComponentAlignment(image, Alignment.MIDDLE_LEFT);
			
		}else if(info.equalsIgnoreCase(SHAConstants.INFO_MSG_ICON)){
		   ThemeResource imgIcon = new ThemeResource("images/info.png");
//			contentLayout.setIcon(icon);
		   Image image = new Image(null, imgIcon);
			contentLayout.addComponent(image);
			contentLayout.setComponentAlignment(image, Alignment.MIDDLE_LEFT);
			
		}else if(info.equalsIgnoreCase(SHAConstants.CONF_MSG_ICON)){
			ThemeResource imgIcon = new ThemeResource("images/question.png");
//			contentLayout.setIcon(icon);
			Image image = new Image(null, imgIcon);
			contentLayout.addComponent(image);
			contentLayout.setComponentAlignment(image, Alignment.MIDDLE_LEFT);
			
		}else if(info.equalsIgnoreCase(SHAConstants.ALERT_MSG_ICON)){
			ThemeResource imgIcon = new ThemeResource("images/warning.png");
//			contentLayout.setIcon(icon);
			Image image = new Image(null, imgIcon);
			contentLayout.addComponent(image);
			contentLayout.setComponentAlignment(image, Alignment.MIDDLE_LEFT);
	...
	protected MessageBox(String info) {
		// Create window
		window = new Window();
		//window.setClosable(false);
		//window.setModal(true);
	...
	public MessageBox withCaption(String caption) {
		...
		window.setClosable(BUTTON_ADD_CLOSE_PER_DEFAULT);
		//window.setClosable(window.setIcon(new ThemeResource("images/cross.png")).true);
		//window.setIcon(new ThemeResource("images/cross.png"));
		//window.removeStyleName("v-window-closebox-disabled");
		window.setCaptionAsHtml(true);
		return this;
	}
	
	public MessageBox withCaptionCust(String caption) {
		window.setCaption(caption);
		window.setClosable(false);
		window.setCaptionAsHtml(true);
		return this;
	...
	public MessageBox withHtmlErrorMessage(String htmlMessage) {
		return withMessage(new Label("<b style = 'color: red;'>"+htmlMessage+ "</b>", ContentMode.HTML));
	}

	
	public MessageBox withHtmlCriticalErrorMessage(String htmlMessage) {
		return withMessage(new Label("<b style ='color: white;'>"+htmlMessage+ "</b>", ContentMode.HTML));
	}
	/**
	...
	public MessageBox withTableMessage(VerticalLayout layout) {
		
			if (layout != null) {
				contentLayout.removeComponent(layout);

				layout.setSizeFull();
				contentLayout.addComponent(layout, contentLayout.getComponentCount());
				contentLayout.setExpandRatio(layout, 1.0f);		
				contentLayout.setComponentAlignment(layout, Alignment.MIDDLE_CENTER);
			}
			return this;
	...
	public MessageBox withButtonSpacer() {
		...
		buttonLayout.addComponent(new Label("&nbsp;", com.vaadin.v7.shared.ui.label.ContentMode.HTML));
		return this;
	}
	...
	public MessageBox withButton(ButtonType buttonType, Runnable runOnClick, ButtonOption... options) {
		...
		if(BUTTON_DEFAULT_ICON_FACTORY.getIcon(buttonType)!=null){
			button.setIcon(BUTTON_DEFAULT_ICON_FACTORY.getIcon(buttonType));
		}
	...
	public MessageBox withOkButton(ButtonOption... options) {
		final Resource imgIcon = new ThemeResource("images/tick.png");
		ButtonOption[] newArray = new ButtonOption[options.length + 1];
		 for(int i=0;i < options.length;i++)
			 newArray[i] = options[i];
		newArray[newArray.length-1] = ButtonOption.icon(imgIcon);
		options = newArray;
	...
	public MessageBox withNoButton(ButtonOption... options) {
		final Resource imgIcon = new ThemeResource("images/cross.png");
		ButtonOption[] newArray = new ButtonOption[options.length + 1];
		 for(int i=0;i < options.length;i++)
			 newArray[i] = options[i];
		newArray[newArray.length-1] = ButtonOption.icon(imgIcon);
	...
	public static MessageBox customizedWindow(Boolean isCritical,String info) {
		return new MessageBox(isCritical,info);
	}
	
	public static MessageBox customizedWindowforConf(String info) {
		return new MessageBox(info);
	}
	...
	public static MessageBox createInfo() {
		...
		return customizedWindow(false, SHAConstants.INFO_MSG_ICON);
	}
	
	public static MessageBox createInfo(Object obj) {
		//return create().withIcon(DIALOG_DEFAULT_ICON_FACTORY.getInfoIcon());
		return customizedWindow(false, SHAConstants.INFO_MSG_ICON);
	}
	...
	public static MessageBox createQuestion() {
		...
		return customizedWindow(false, SHAConstants.CONF_MSG_ICON);
	}
	
	public static MessageBox createQuestionCust() {
		//return create().withIcon(DIALOG_DEFAULT_ICON_FACTORY.getQuestionIcon());
		return customizedWindowforConf(SHAConstants.CONF_MSG_ICON);
	}
	...
	public static MessageBox createWarning() {
		...
		return customizedWindow(false, SHAConstants.ALERT_MSG_ICON);
	}
	...
	public static MessageBox createError() {
		return customizedWindow(false, SHAConstants.ERROR_MSG_ICON);
	}
	
	/**
	 * Shows the dialog.  
	 */
	public MessageBox open() {	
	...
	public static MessageBox createCritical() {
		return customizedWindow(true,SHAConstants.ERROR_MSG_ICON);
		//return criticalWindow(true);
	}
	
	public void setMessageLayoutStyle(String styleName) {
