...
public class OMPClaimProcessCheckerViewImpl extends AbstractMVPView implements OMPClaimProcessCheckerView{
	
	@Inject
	OMPClaimProcessCheckerUI searchForm;
	@Inject
	OMPClaimProcessCheckerDetailTable searchResultTable;
...
private VerticalSplitPanel mainPanel;
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof OMPClaimProcessCheckerDetailTable)
			{
				((OMPClaimProcessCheckerDetailTable) comp).removeRow();
			}
		}
	...
	public void doSearch() {
		OMPClaimProcessCheckerFormDto searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		fireViewEvent(OMPClaimProcessCheckerPresenter.SUBMIT_SEARCH, searchDTO,userName,passWord);
