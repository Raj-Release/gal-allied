...
public class PreviousPreAuthDetailsTable extends GBaseTable<PreviousPreAuthTableDTO> {
	...
	public static final Object[] COLUMN_HEADER_PA = new Object[] {
		"serialNumber",
		"referenceNo",
		"referenceType",
		"docReceivedFrom",
	...
	public void setPAColumns(){
		table.setVisibleColumns(COLUMN_HEADER_PA);
...
//						masterService);
	...
	/*	List<PreviousClaimsTableDTO> previousClaimDTOList = preauthService
		...
		if(ReferenceTable.getGMCProductList().containsKey(preauthDTO.getNewIntimationDTO().getPolicy().getProduct().getKey())){
			insuredSumInsured = dbCalculationService.getInsuredSumInsuredForGMC(preauthDTO.getPolicyDto()
						.getKey(), preauthDTO.getNewIntimationDTO().getInsuredPatient().getKey(), preauthDTO.getNewIntimationDTO().getPolicy().getSectionCode());
		}else{
...
//			this.diagnosisListenerTableObj.addList(filledDTO);
	...
	public void sumInsuredCalculation(Map<String, Object> values,Preauth preauth){
		...
		values.put("", preauth.getClaim().getIntimation().getPolicy().getProduct().getKey());
