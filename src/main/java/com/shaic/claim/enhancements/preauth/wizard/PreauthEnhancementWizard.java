...
public interface PreauthEnhancementWizard extends GMVPView{
	...
	void genertateFieldsBasedOnOtherBenefits(PreauthDTO beanDto);
	...
	void generateWithdrawAndRejectLayout(BeanItemContainer<SelectValue> withdrawRejectContainer);
	...
	void generatePanCardLayout();

	void checkPanCardDetails(Boolean isAvailable);
	
	void generatePTCACABGLayout(Boolean value);
	
	void generatePTCACABGLayoutDelete();

	void validationForLimitAmount();
	...
	void setPreauthEnhnProcedureValues(BeanItemContainer<SelectValue> procedures);
...
/*	void genertateFieldsBasedOnNegotiation(Boolean isChecked);*/
	...
	void generateButtonLayoutBasedOnScoring(PreauthEnhancementButtons button);
	
	void setQueryRemarks(String qryRemarks);
