...
public class PreauthEnhancementPreviousClaimDetailsPage extends PreMedicalPreauthPreviousClaimsPageUI implements WizardStep<PreauthDTO> {
	private static final long serialVersionUID = 4764292929795862668L;
...
//		viewButton = getViewButton();
		...
		if(this.bean.getPreviousClaimsList() != null && this.bean.getPreviousClaimsList().isEmpty()){
			...
			fireViewEvent(PreauthEnhancemetWizardPresenter.ENHANCEMENT_PREVIOUS_CLAIM_FOR_POLICY,bean);
		}}
		...
		viewType.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if (viewType.getValue().equals(POLICY_WISE)) {						
					...
					fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean,SHAConstants.POLICY_WISE);
				} else if (viewType.getValue().equals(INSURED_WISE)) {						
					preauthPreviousClaimsTableObj.setCaption("Insured Wise");
					//fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean.getPolicyDto().getInsuredId());
					fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean,SHAConstants.INSURED_WISE);
				} else if (viewType.getValue().equals(RISK_WISE)) {
					preauthPreviousClaimsTableObj.setCaption("Risk Wise");
					//preauthPreviousClaimsTableObj.setTableList(bean.getPreviousClaimsList());
					fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean,SHAConstants.RISK_WISE);
				}
			}
...
//						preauthPreviousClaimsTableObj.setCaption("Insured Wise");
//						//fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean.getPolicyDto().getInsuredId());
//						fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean);
//					} else if (viewType.getValue().equals(RISK_WISE)) {
//						preauthPreviousClaimsTableObj.setCaption("Risk Wise");
//						preauthPreviousClaimsTableObj.setTableList(bean.getPreviousClaimsList());
//					}
		...
		cmbRelapseOfIllness.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if(event.getProperty() != null && (SelectValue)event.getProperty().getValue() != null ){//&& ((SelectValue)event.getProperty().getValue()).getId() == ReferenceTable.COMMONMASTER_YES) {
					fireViewEvent(PreauthEnhancemetWizardPresenter.PREAUTH_RELAPSE_OF_ILLNESS_CHANGED, bean, (SelectValue)event.getProperty().getValue());
				}
