 ...
 *
 */
public class UploadedDocumentsListenerTable  extends ViewComponent { 
...
//						button.setIcon(new ThemeResource("images/BillEntry.png"));
						...
						*/
							...
							{
								fireViewEvent(ClaimRequestMedicalDecisionPagePresenter.MA_LOAD_BILL_DETAILS_VALUES, uploadDocDTO);
							}
							else if((SHAConstants.BILLING).equalsIgnoreCase(presenterString))
							{
								fireViewEvent(BillingReviewPagePresenter.BILLING_LOAD_BILL_DETAILS_VALUES, uploadDocDTO);
							}
							else if((SHAConstants.FINANCIAL).equalsIgnoreCase(presenterString))
							{
								fireViewEvent(FinancialReviewPagePresenter.FA_LOAD_BILL_DETAILS_VALUES, uploadDocDTO);
							}
...
//		Button button = (Button)uploadDocDTO.getButtonMap().get(uploadDocDTO.getSeqNo());
	...
	@SuppressWarnings("unchecked")
	public void setBillClassificationsDynamicallyForOtherBenefits(Boolean isNeededPre, Boolean isNeededPost, Boolean isNeededHosp,Boolean isNeededOtherBenefit) {
		BeanItemContainer<SelectValue> object = (BeanItemContainer<SelectValue>) referenceData.get(SHAConstants.ALL_BILL_CLASSIFICATIONS);
		List<SelectValue> itemIds = object.getItemIds();
		BeanItemContainer<SelectValue> newValues = new BeanItemContainer<SelectValue>(SelectValue.class);
		List<SelectValue> selectedValues = new ArrayList<SelectValue>();
		for (SelectValue selectValue : itemIds) {
			...
			if(isNeededOtherBenefit & selectValue.getId().equals(ReferenceTable.OTHER_BENEFIT) ) {
				selectedValues.add(selectValue);
			}			
		}
	...
	public void setRoomType(SelectValue selected){
		List<UploadDocumentDTO> itemIds = (List<UploadDocumentDTO>)table.getItemIds();
		for (UploadDocumentDTO uploadDocumentDTO : itemIds) {
			uploadDocumentDTO.setRoomCategory(selected);
		}
	}
}
