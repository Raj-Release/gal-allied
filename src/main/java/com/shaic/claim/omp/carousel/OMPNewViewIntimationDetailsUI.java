...
//				viewDetails.getViewPolicy(dtoBean.getIntimationno());
		 ...
		 viewMedButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(dtoBean.getIntimationno()!= null){
					String merUrl = viewDetails.getOMPMERDetails(dtoBean.getIntimationno());
...
//	if((claim.getStatus().getKey().intValue() == ReferenceTable.CLAIM_REGISTERED_STATUS) || (claim.getStage().getKey().intValue() == ReferenceTable.CLAIM_SUGGEST_REJECTION_STATUS)){
	...
	claimNumber.setReadOnly(true);
		...
		if(isTPAUserLogin()){
			horizontalLayout = new HorizontalLayout(acknowledgementDetailsButton,earlierAcknowledgementDetailsButton,riskButton,policyDetails,viewHistoryButton,viewClaimantButton,viewDocButton);
