...
@UIScoped
public class SearchGmcDailyReport extends ViewComponent {
	...
	@Inject
	private GmcDailyReportTable policywiseClaimTable;
	...
	private Panel buildSearchPanel(){
		...
		searchPanel = new Panel("Daily Report - GMC");
	...
	private VerticalLayout buildSearchLayout(){
		...
		searchFormFieldLayout = new FormLayout(clmTypeCmb);
				...
				if(validateFields())
					{
						...
						fireViewEvent(GmcDailyReportPresenter.SEARCH_GMC_CLMS_DAILY_NEW, filters);
					}
			}
		});
...
//		searchVerticalLayout.setComponentAlignment(buttonLayout, Alignment.MIDDLE_CENTER);
	...
	public void showTable(List<GmcDailyReportDto> claimList){
	...
	private void addExportButton(){
		...
		exportBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				excelExport.setReportTitle("GMC Daily Report");
	...
	public void clearSearchTable(){
		policywiseClaimTable.setTableList(new ArrayList<GmcDailyReportDto>());
		if(exportBtn != null){
			buttonLayout.removeComponent(exportBtn);
			buttonLayout.removeComponent(dummyLabel);
			exportBtn = null;
