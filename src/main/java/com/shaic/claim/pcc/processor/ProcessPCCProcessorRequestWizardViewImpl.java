...
public class ProcessPCCProcessorRequestWizardViewImpl extends AbstractMVPView implements ProcessPCCProcessorRequestWizardView {

	@Inject
	private Instance<ProcessPccProcessorRequestPage> processPccProcessorRequestPageInst;

	private ProcessPccProcessorRequestPage processPccProcessorRequestPage;
	...
	public void initView(PccDetailsTableDTO pccDetailsTableDTO){
		...
		processPccProcessorRequestPage = processPccProcessorRequestPageInst.get();
		processPccProcessorRequestPage.init(pccDetailsTableDTO,presenterString);
		...
		VerticalLayout mainVertical = new VerticalLayout(intimationDetailsCarousel,commonButtonLayout(),processPccProcessorRequestPage,buttonHor);
		mainVertical.setComponentAlignment(processPccProcessorRequestPage, Alignment.BOTTOM_CENTER);
	...
	public VerticalLayout commonButtonLayout(){
		...
		viewDetails.initView(bean.getIntimationDto().getIntimationId(),null, ViewLevels.PREAUTH_MEDICAL,"PCC Processor");
	...
	public void buildSuccessLayout() {
		...
		homeButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				fireViewEvent(MenuItemBean.PCC_PROCESSOR, null);
	...
	private void addListener() {
		...
		submitBtn.addClickListener(new Button.ClickListener() {
			...
			@Override
			public void buttonClick(ClickEvent event) {
				if (processPccProcessorRequestPage.validatePage()) {
					PccDTO pccDTO = processPccProcessorRequestPage.getvalues();
					if(pccDTO !=null){
						String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
						fireViewEvent(ProcessPccProcessorRequestPresenter.SUBMIT_PCCCPROCESSOR_DETAILS,pccDTO,userName,bean);
					}			
				} 
			}
		});
			...
			public void buttonClick(ClickEvent event) {
				...
				homeButton.addClickListener(new ClickListener() {
					...
					public void buttonClick(ClickEvent event) {
						...
						fireViewEvent(MenuItemBean.PCC_PROCESSOR, true);
					}
				});
	...
	public void generateapproveLayout() {
		processPccProcessorRequestPage.generateapproveLayout();
	...
	public void generateQuerryLayout() {
		processPccProcessorRequestPage.generateQuerryLayout();
	...
	public void generateDisapproveLayout() {
		processPccProcessorRequestPage.generateDisapproveLayout();
	...
	public void generateResponseLayout() {
		processPccProcessorRequestPage.generateResponseLayout();

	}

	@Override
	public void addUserDetails(BeanItemContainer<SelectValue> userDetailsContainer) {
		processPccProcessorRequestPage.addUserDetails(userDetailsContainer);
	}
