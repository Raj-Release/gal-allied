...
import com.shaic.claim.processdatacorrection.viewtable.ViewActualProcedureCorrectionTable;
...
public class DataCorrectionPriorityViewImpl extends AbstractMVPView implements DataCorrectionPriorityView {
	...
	public void initView(ProcessDataCorrectionDTO bean) {
		...
		addStyleName("view");
		setSizeFull();
		fireViewEvent(DataCorrectionPriorityPresenter.SET_TABLE_DATA_PRIORITY,null);
		...
		specialityCorrectionObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		actualSpecialityObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		diganosisCorrectionObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		actualdiganosisObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		treatingCorrectionObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		actualtreatingObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		if(bean.getTreatmentType() !=null && 
			...
			procedureCorrectionObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
			...
			actualprocedureObj.init(SHAConstants.DATA_VALIDATION_PRIORITY);
			...
			implantCorrectionObj.initView(bean.getImplantApplicable(),bean.getImplantCorrectionDTOs(),SHAConstants.DATA_VALIDATION_PRIORITY);
...
//			}
		...
		viewDetails.initView(newIntimationDTO.getIntimationId(),bean.getTransactionKey(), ViewLevels.PREAUTH_MEDICAL,SHAConstants.DATA_VALIDATION_PRIORITY);
		...
		submitBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(isvalid()){
					...
					fireViewEvent(DataCorrectionPriorityPresenter.SUBMIT_BUTTON_DATA_CODING_PRIORITY,bean,userName);	
				}
				...
				ConfirmDialog dialog = ConfirmDialog.show(getUI(),"Confirmation", "Are you sure You want to Cancel ?",
					...
					public void onClose(ConfirmDialog dialog) {
						if (!dialog.isConfirmed()) {
							String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
							dbCalculationService.dataCoadingPriorityRelease(userName,null,"CANCEL",bean.getCoadingKey());
							fireViewEvent(MenuItemBean.DATA_CODING_DATA_CORRECTION_PRIORITY, true);
							clearObjects();
						} else {
							dialog.close();
						}
	...
	public void buildSuccessLayout() {
		...
		StringBuffer successLabel = new StringBuffer("Data validation Priority Record Saved Successfully.");
	...
	public void showProposedScoringView(){
		...
		scoringAndPPTabUI.setScreenName(SHAConstants.DATA_VALIDATION_PRIORITY);
	...
	public void clearObjects(){
		...
		verticalMain = null;
		fireViewEvent(DataCorrectionPriorityPresenter.REFERENCE_DATA_CLEAR_PRIORITY,null);
	}
