...
public class SearchProcessDataCorrectionPriorityViewImpl extends AbstractMVPView implements SearchProcessDataCorrectionPriorityView ,Searchable {

	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private SearchProcessDataCorrectionPriorityForm searchForm;
	...
	public void list(ProcessDataCorrectionDTO tableRows) {
		if(null != tableRows)
		{	
			fireViewEvent(MenuItemBean.PROCESS_DATA_CORRECTION_PRIORITY_VIEW,tableRows);
		}
		else
		{
			Label successLabel = new Label("<b style = 'color: black;'>No Records found.</b>", ContentMode.HTML);			
			Button homeButton = new Button("Data Validation Priority");
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.DATA_CODING_DATA_CORRECTION_PRIORITY, null);					
				}
			});		
		}
	}
	...
	public void doSearch() {
		...
		fireViewEvent(SearchProcessDataCorrectionPriorityPresenter.SEARCH_INTIMATION_DATA_CORRECTION_PRIORITY, null,userName);
	}
