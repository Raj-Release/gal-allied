...
public class SearchProcessDataCorrectionService {
	...
	@EJB
	private SearchProcessDataCorrectionPriorityService correctionPriorityService;
		...
		if(newPedValidationTableListDto !=null && !newPedValidationTableListDto.isEmpty()){
			...
			for(DiagnosisDetailsTableDTO detailsTableDTO : newPedValidationTableListDto){
				...
				if (detailsTableDTO.getIcdCode().getId() != null 
					...
					hasTOD = true;
				}else if (correctionDTO.getIcdCode().getId() != null 
						&& correctionDTO.getIcdCode().getId().equals(SHAConstants.COVID_19_ICD_IDENT_KEY)) {
					hasTOD = true;
				}
...
//						procedureCorrectionDTO.setProposedSpeciality(procedureDTO.getSpeciality());
			...
			for(ProcedureCorrectionDTO correctionDTO:procedureCorrectionDTOs){
				if(correctionDTO.getHasChanges()){
					...
					if(procedure !=null){
						...
						if(correctionDTO.getProposedProcedureName() !=null 
								&& correctionDTO.getProposedProcedureName().getValue() != null 
								&& correctionDTO.getProposedProcedureName().getValue().toUpperCase().equalsIgnoreCase("OTHERS") &&
								correctionDTO.getProposedNewProcedureName() !=null &&
								!correctionDTO.getProposedNewProcedureName().isEmpty()){
							procedure.setNewProceudreName(correctionDTO.getProposedNewProcedureName());
						}else {
							procedure.setNewProceudreName(null);
						}
	...
	public void saveDataCorrection(ProcessDataCorrectionDTO dataCorrectionDTO,String userID){
		...
		if(dataCorrectionDTO.getIsCovid19VariantChanged()){
			haschanges= true;
			if(reimbursement !=null){
				if(dataCorrectionDTO.getActualCovid19Variant() !=null){
					MastersValue mastersValueCovid = new MastersValue();
					mastersValueCovid.setKey(dataCorrectionDTO.getActualCovid19Variant().getId());
					if(reimbursement.getCovid19Variant() != null){
						reimbursement.setOldCovid19Variant(reimbursement.getCovid19Variant());
						reimbursement.setCovid19Variant(mastersValueCovid);
					}
					else{
						reimbursement.setCovid19Variant(mastersValueCovid);
					}
				...
				if(preauth!=null){
					if(dataCorrectionDTO.getActualCovid19Variant()!=null){
						if(reimbursement.getCovid19Variant() !=null){
							preauth.setOldCovid19Variant(reimbursement.getCovid19Variant());
						}
						MastersValue mastersValueActualCovid = new MastersValue();
						mastersValueActualCovid.setKey(dataCorrectionDTO.getActualCovid19Variant().getId());
						preauth.setCovid19Variant(mastersValueActualCovid);
					}
		...
		//
		//cocktailDrug
		if(dataCorrectionDTO.getIsCocktailDrugChanged()){
			haschanges= true;
			if(reimbursement !=null){
				if(dataCorrectionDTO.getActualCocktailDrug() !=null){
					MastersValue mastersValueCocktailDrug = new MastersValue();
					mastersValueCocktailDrug.setKey(dataCorrectionDTO.getActualCocktailDrug().getId());
					if(reimbursement.getCocktailDrug() != null){
						reimbursement.setOldCocktailDrug(reimbursement.getCocktailDrug());
						reimbursement.setCocktailDrug(mastersValueCocktailDrug);
					}
					else{
						reimbursement.setCocktailDrug(mastersValueCocktailDrug);
					}
				...
				if(preauth!=null){
					if(dataCorrectionDTO.getActualCocktailDrug()!=null){
						if(reimbursement.getCocktailDrug() !=null){
							preauth.setCocktailDrug(reimbursement.getCocktailDrug());
						}
						MastersValue mastersValueActualCocktailDrug = new MastersValue();
						mastersValueActualCocktailDrug.setKey(dataCorrectionDTO.getActualCocktailDrug().getId());
						preauth.setCocktailDrug(mastersValueActualCocktailDrug);
					}
			...
			if(dataCorrectionDTO.getActualadmissionType() !=null && claim !=null && claim.getClaimType() !=null){
	    		if(claim.getClaimType().getKey().equals(ReferenceTable.REIMBURSEMENT_CLAIM_TYPE_KEY)){
	        		...
	        		if(preauth.getTypeOfAdmission() !=null 
	        			...
	        			preauth.setOldTypeOfAdmission(reimbursement.getTypeOfAdmission());
					...
					entityManager.flush();
					entityManager.clear();
					log.info("------DiganosisPreauth------>"+preauth+"<------------");
	        	}
	    	}
	...
	public ProcessDataCorrectionDTO getCorrectionViewDatas(String intimationNO){
		...
		if(claim.getDcCoadingFlag() !=null 
			...
			if(reimbursement == null){
				preauth = preauthService.getLatestPreauthByClaimKey(claim.getKey());
				transactionKey = preauth.getKey();
			}else if(reimbursement !=null){
				transactionKey = reimbursement.getKey();
			}
			...
			if(transactionKey != null){
				...
				if(reimbursement !=null){
					...
					correctionPriorityService.setRODroomCat(transactionKey,dataCorrectionDTO,true,"C");
				}
				...
				if(dataCorrectionDTO.getIcdExclusionReason() == null){
					Preauth preauthD = preauthService.getLatestPreauthByClaimKey(claimKey);
					if(preauthD != null && preauthD.getReasonForIcdExclusion() != null){
						dataCorrectionDTO.setIcdExclusionReason(preauthD.getReasonForIcdExclusion());
					}
				}
				return dataCorrectionDTO;
			}	
				...
				if(treatingDoctorDetails.getHasChanges()){
					if(treatingDoctorDetails.getKey() !=null){
						...
						if(details !=null){
							...
							if(treatingDoctorDetails.getTreatingDoctorSignature() != null && treatingDoctorDetails.getTreatingDoctorSignature().getValue() != null){
								details.setTreatingDoctorSignature(treatingDoctorDetails.getTreatingDoctorSignature().getValue());
							}
