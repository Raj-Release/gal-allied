...
public class SearchAdviseOnPEDViewImpl  extends AbstractMVPView implements SearchAdviseOnPEDView, Searchable  {
	

	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private SearchAdviseOnPEDForm searchForm;
	
	@Inject
	private SearchAdviseOnPEDTable searchResultTable;
...
*/
		...
		else
		{
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.ADVISE_ON_PED, null);
	...
	public void doSearch() {
		SearchAdviseOnPEDFormDTO searchDTO = searchForm.getSearchDTO();
		...
		fireViewEvent(SearchAdviseOnPEDPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchAdviseOnPEDTable)
			{
				((SearchAdviseOnPEDTable) comp).removeRow();
			}
		}
	}
