...
import com.shaic.claim.lumen.policyupload.LumenPolicyUploadDocumentsViewImpl;
...
public class InitiateLumenPolicyRequestWizardImpl extends AbstractMVPView implements InitiateLumenPolicyRequestWizard{

	@Inject
	private Instance<InitiateLumenPolicyCarousel> carouselInstance;

	@Inject
	private ViewDetails viewDetails;

	private VerticalLayout mainPanel;
	private LumenPolicySearchResultTableDTO dtoBean;
	...
	@Inject
	private LumenPolicyUploadDocumentsViewImpl uploadDocumentView;
	...
	public void initView(LumenPolicySearchResultTableDTO resultDTO) {
		this.dtoBean =null;
		this.dtoBean = resultDTO;
		mainPanel = new VerticalLayout();

		InitiateLumenPolicyCarousel lumenCarousel = carouselInstance.get();
		lumenCarousel.init(dtoBean, "Initiate Lumen Request");
		mainPanel.addComponent(lumenCarousel);

		viewDetails.initView(null, resultDTO.getPolicyKey(), ViewLevels.LUMEN_TRAILS, false,"Lumen Policy Trails Page loaded",false);
	...
	private AbsoluteLayout commonButtonsLayout(){
		...
		txtIntimationNo = new TextField("Policy Number");
		txtIntimationNo.setValue(this.dtoBean.getPolicyNumber());
		txtIntimationNo.setReadOnly(true);
		
		cmbInsuredName = new ComboBox("Insured Name");
		BeanItemContainer<SelectValue> insuredContainer = lumenService.getInsuredNameList(dtoBean);
		cmbInsuredName.setContainerDataSource(insuredContainer);
...
//		cmbErrorType.setVisible(false);
		...
		FormLayout firstForm1 = new FormLayout(txtEmployeeName,txtEmployeeId,txtIntimationNo,cmbInsuredName,cmbLumenType,txtComments);
		...
		searchlumen_layout.setHeight("360px");
	...
	public AbsoluteLayout addFooterButtons(){
		...
		cancelButton.addClickListener(new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				fireViewEvent(InitiateLumenPolicyRequestWizardPresenter.POLICY_CANCEL_REQUEST,null);
			}
		});
		...
		submitButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(!validatePage()){
					fireViewEvent(InitiateLumenPolicyRequestWizardPresenter.POLICY_SUBMIT_REQUEST,dtoBean);
				}else{
...
//		}
		...
		this.dtoBean.setSelectedInsuredName(cmbInsuredName.getValue().toString());
		try {
			String refNumber = lumenService.savePolicyLumenRequest(this.dtoBean);
	...
	private boolean validatePage() {
		...
		if(cmbInsuredName.getValue() == null){
			hasError = true;
			errMsg.append("Please select Insured Name </br>");
		}
