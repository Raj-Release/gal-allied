...
@UIScoped
public class LumenSearchQueryToMISForm extends SearchComponent<LumenSearchReqFormDTO>{
...
//	private ComboBox cmbCPUOffice;
	...
	@Inject
	SearchQueryToMISResultTable lumenTable;
	...
	public void init() {
		...
		mainPanel.setCaption("Lumen Query Request (To MIS Team)");
...
//		cmbCPUOffice = binder.buildAndBind("CPU Office","cmbCPUOffice",ComboBox.class);
		...
		cmbEmpName =  new ComboBoxMultiselect("Employee ID/Name"); //binder.buildAndBind("Query Initiated By","cmbEmpName",ComboBox.class);
		cmbSource = binder.buildAndBind("Query Initiated From","cmbSource",ComboBox.class);
		cmbStatus = binder.buildAndBind("Status","cmbStatus",ComboBox.class);

		FormLayout formLayoutLeft = new FormLayout(intimationNumber,policyNumber,cmbCPUOffice);
		FormLayout formLayoutRight = new FormLayout(cmbSource,cmbEmpName,cmbStatus);
...
//		cmbCPUOffice.setItemCaptionPropertyId("value");
		...
		BeanItemContainer<SelectValue> stageValueContainer = new BeanItemContainer<SelectValue>(SelectValue.class);
		SelectValue stg1 = new SelectValue();
		stg1.setValue("Process Lumen Level 1");
		SelectValue stg2 = new SelectValue();
		stg2.setValue("Process Coordinator Lumen");
		SelectValue stg3 = new SelectValue();
		stg3.setValue("Process Lumen Level 2");
		stageValueContainer.addBean(stg1);
		stageValueContainer.addBean(stg2);
		stageValueContainer.addBean(stg3);		
		cmbSource.setContainerDataSource(stageValueContainer);
		cmbSource.setItemCaptionMode(ItemCaptionMode.PROPERTY);
		cmbSource.setItemCaptionPropertyId("value");
		
		
		/*BeanItemContainer<SelectValue> statusValueContainer = new BeanItemContainer<SelectValue>(SelectValue.class);
		SelectValue queryselect = new SelectValue();
		queryselect.setId(1L);
		queryselect.setValue("REFER TO MIS");
		statusValueContainer.addBean(queryselect);		
		cmbStatus.setContainerDataSource(statusValueContainer);
		cmbStatus.setItemCaptionMode(ItemCaptionMode.PROPERTY);
		cmbStatus.setItemCaptionPropertyId("value");		
		List<SelectValue> defaultStatusType = (List<SelectValue>)cmbStatus.getContainerDataSource().getItemIds();
		cmbStatus.setValue(defaultStatusType.get(0));
		cmbStatus.setReadOnly(true);*/
		
		BeanItemContainer<SelectValue> lumenStatusTypes = lumenService.getMISQueryStatus();
