...
public class ProcessCoordinatorWizardImpl extends AbstractMVPView implements ProcessCoordinatorWizard{
	...
	public void initView(LumenRequestDTO resultDTO) {
		...
		lumenCarousel.init(dtoBean, "Process Lumen Request - Coordinator");
		...
		if(listOfQryObj != null && onlylistOfReplyObj != null){
			if(listOfQryObj.size() != onlylistOfReplyObj.size()){
				...
				functionalbuttonLayout.init(false, false, true, false, false, false,"Process Coordinator");
			}else{
				functionalbuttonLayout.init(true, false, false, true, false, true,"Process Coordinator");
			}
		}else{
			functionalbuttonLayout.init(true, false, false, true, false, true,"Process Coordinator");
		}
		...
		/*if(listOfQryObj != null && listOfQryObj.size() > 0){
			functionalbuttonLayout.setQueryRaisedRole(listOfQryObj.get(0).getQueryRaisedRole());
		}*/
		if(listOfQryObj != null && listOfQryObj.size() > 0){
			for(LumenQueryDTO rec : listOfQryObj){
	...
	public VerticalLayout addFooterButtons(){
		...
		cancelButton.addClickListener(new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				fireViewEvent(ProcessCoordinatorWizardPresenter.CANCEL_COORDINATOR_REQUEST,null);
			}
		});
		...
		submitButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(!validatePage()){
					fireViewEvent(ProcessCoordinatorWizardPresenter.SUBMIT_COORDINATOR_REQUEST,dtoBean);
				}else{
		...
		ConfirmDialog dialog = ConfirmDialog.show(getUI(), "Confirmation",
			...
			public void onClose(ConfirmDialog dialog) {
				if (dialog.isCanceled() && !dialog.isConfirmed()) {
					// YES
					fireViewEvent(MenuItemBean.LUMEN_REQUEST_CO_ORDINATOR, null);
				}
			}
		});
	...
	public void submitLumenRequest(){
		...
		if(actionName.equals("Reply")){
			dtoBean.setCoordReplyRemarks(functionalbuttonLayout.getReplyComments().getValue());
			dtoBean.setLevel1ReplyTo(String.valueOf(functionalbuttonLayout.getCmbReplyTo().getValue()));
		}
		if(actionName.equals("Approve")){
			dtoBean.setCoordApprovalRemarks(functionalbuttonLayout.getApproveComments().getValue());
			dtoBean.setLevel1ApproveTo(String.valueOf(functionalbuttonLayout.getCmbApproveTo().getValue()));
		}
		...
		try {
			lumenService.saveLumenCoordinatorRequest(dtoBean);
			String msg = "Lumen Coordinator Changes Submitted Successfully </br>";
			showSubmitMessagePanel(msg);
		} catch (Exception e) {
			String msg = "Exception Occurred While merging Coordinator changes </br>";
			showSubmitMessagePanel(msg);
			e.printStackTrace();
		}
	...
	private boolean validatePage() {
		...
		if(StringUtils.isBlank(actionName)){
			hasError = true;
			errMsg.append("Please perform functional operations like Approve/Reply/Query/Refer to MIS before submit </br>");
		}else{
			if(actionName.equals("Query")){
			...
			if(actionName.equals("Approve")){
				if(functionalbuttonLayout.getCmbApproveTo().getValue() == null){
					hasError = true;
					errMsg.append("Please Select Approve To value. </br>");
				}
	...
	public void showReplyFromMISView(){
		...
		misReplyDetails.setScreenName("ProcessCoordinator");
