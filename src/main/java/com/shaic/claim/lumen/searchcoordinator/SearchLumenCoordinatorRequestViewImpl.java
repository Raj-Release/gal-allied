...
public class SearchLumenCoordinatorRequestViewImpl extends AbstractMVPView implements SearchLumenCoordinatorRequestView{

	@Inject
	private LumenSearchCoordinatorReqForm lumenForm;
	
	@Inject
	private SearchCoordinatorResultTable lumenTable;
...
//		lumenTable.init("", false, false);
	...
	public void doSearch() {
		if(!validatePage()){
			...
			//searchDTO.setDocNameList((List<SelectedDoctorDTO>) lumenForm.getSelectedDoctorTable().getTable().getItemIds());
			Pageable pageable = null;
			lumenTable.addSearchListener(this);
			pageable = lumenTable.getPageable();
			searchDTO.setPageable(pageable);		
			fireViewEvent(SearchLumenCoordinatorRequestPresenter.DO_LUMEN_REQ_COORDINATOR_SEARCH, searchDTO,userName,passWord);
		}else{
