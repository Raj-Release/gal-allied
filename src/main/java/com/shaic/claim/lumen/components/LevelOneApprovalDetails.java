...
public class LevelOneApprovalDetails extends ViewComponent{
	...
	private LumenTrials trialsDataObj;
	...
	public void init(LumenRequestDTO resultDTO, String caption) {
		...
		trialsDataObj = lumenService.getLevelOneDataFromTrails(dtoObj, "Process Lumen Level 1");
	...
	private FormLayout buildCarousel() {
		...
		if(trialsDataObj != null){
			employeeName.setValue(dtoObj.getRemarksEmpName());
		}else{
		...
		if(trialsDataObj != null){
			employeeId.setValue(trialsDataObj.getCreatedBy());
		}else{
			employeeId.setValue("");
		}
		...
		if(trialsDataObj != null){
			if (null != trialsDataObj.getStatusRemarks() && !trialsDataObj.getStatusRemarks().isEmpty()){
				comments.setValue(trialsDataObj.getStatusRemarks());
			}
		}else{
			comments.setValue("");
		}
	...
	private ShortcutListener getShortCutListenerForRemarks(final TextArea txtFld)
	{
		ShortcutListener listener =  new ShortcutListener("Initiator Remarks",KeyCodes.KEY_F8,null) {
			...
			public void handleAction(Object sender, Object target) {
				...
				String strCaption = "Level 1 Approval Remarks";
