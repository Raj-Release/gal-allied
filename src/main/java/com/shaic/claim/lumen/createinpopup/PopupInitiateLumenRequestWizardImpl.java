...
public class PopupInitiateLumenRequestWizardImpl extends AbstractMVPView implements PopupInitiateLumenRequestWizard{
	...
	public void initView(LumenSearchResultTableDTO resultDTO, Window popup, String screeName) {
		this.dtoBean =null;
		this.dtoBean = resultDTO;
		this.popup = popup;
		this.initiateScreenName = screeName;
		mainPanel = new VerticalLayout();
		mainPanel.removeAllComponents();

		InitiateLumenCarousel lumenCarousel = carouselInstance.get();
		lumenCarousel.init(dtoBean, "Intimation / Policy Details");
		lumenCarousel.getPanel().setHeight("40%");
		lumenCarousel.getPanel().setStyleName("girdBorder");
		...
		uploadLayout.setHeight("50px");
		uploadLayout.setComponentAlignment(uploadButton, Alignment.MIDDLE_RIGHT);
...
//		cmbErrorType.setVisible(false);
	...
	public AbsoluteLayout addFooterButtons(){
		...
		cancelButton.addClickListener(new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				fireViewEvent(PopupInitiateLumenRequestWizardPresenter.POPUP_CANCEL_REQUEST,null);
			}
		});
		...
		submitButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(!validatePage()){
					fireViewEvent(PopupInitiateLumenRequestWizardPresenter.POPUP_SUBMIT_REQUEST,dtoBean);
				}else{
...
//		}
		...
		if(initiateScreenName.equals("SSA Reimbursement") || initiateScreenName.equals("SSA Cashless")){
			this.dtoBean.setSpecialistStage(getSpecialistStage());
		}
		try {
			String refNumber = lumenService.savePopupLumenRequest(this.dtoBean, initiateScreenName);
...
//		}
	...
	public void showSubmitMessagePanel(String messageInfo){
		...
		HashMap<String, String> buttonsNamewithType = new HashMap<String, String>();
		buttonsNamewithType.put(GalaxyButtonTypesEnum.OK.toString(), "Close");
		HashMap<String, Button> messageBoxButtons = GalaxyAlertBox
				.createInformationBox(messageInfo, buttonsNamewithType);
