...
public class ProcessLevelOneWizardImpl extends AbstractMVPView implements ProcessLevelOneWizard{
	...
	public void initView(LumenRequestDTO resultDTO) {
		...
		lumenCarousel.init(dtoBean, "Process Lumen Request - Level I");
		...
		if(listOfQryObj != null && onlylistOfReplyObj != null){
			if(listOfQryObj.size() != onlylistOfReplyObj.size()){
				...
				functionalbuttonLayout.init(false, false, true, false, false, false,"Process Level I");
			}else{
				functionalbuttonLayout.init(true, true, false, true, true, true,"Process Level I");
			}
		}else{
			functionalbuttonLayout.init(true, true, false, true, true, true,"Process Level I");
		}
	...
	public VerticalLayout addFooterButtons(){
		...
		cancelButton.addClickListener(new Button.ClickListener() {
			@Override
			public void buttonClick(ClickEvent event) {
				fireViewEvent(ProcessLevelOneWizardPresenter.CANCEL_LEVEL_I_REQUEST,null);
			}
		});
		...
		submitButton.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(!validatePage()){
					fireViewEvent(ProcessLevelOneWizardPresenter.SUBMIT_LEVEL_I_REQUEST,dtoBean);
				}else{
		...
		ConfirmDialog dialog = ConfirmDialog.show(getUI(), "Confirmation",
			...
			public void onClose(ConfirmDialog dialog) {
				if (dialog.isCanceled() && !dialog.isConfirmed()) {
					// YES
					fireViewEvent(MenuItemBean.LUMEN_REQUEST_LEVEL_I, null);
				}
			}
		});
	...
	public void submitLumenRequest(){
		...
		if(actionName.equals("Approve")){
			if (null != functionalbuttonLayout.getApproveComments().getValue() && !functionalbuttonLayout.getApproveComments().getValue().isEmpty()) {
...
//		}
		...
		if(actionName.equals("Approve") && (participants.size() == 0 && lapse.size() == 0)){
			...
			try {
				lumenService.saveLumenLevelOneRequest(dtoBean);
				String msg = "Lumen Level 1 Request Changes Submitted Successfully </br>";
				showSubmitMessagePanel(msg);
			} catch (Exception e) {
				String msg = "Exception Occurred While merging Lumen level 1 Request </br>";
				showSubmitMessagePanel(msg);
				e.printStackTrace();
			}
		}
...
//		}
		...
		if(StringUtils.isBlank(actionName)){
			hasError = true;
			errMsg.append("Please perform functional operations like Approve/Reject/Reply/Query/Close/Refer to MIS before submit </br>");
		}else{
			if(actionName.equals("Close")){
	...
	public void showReplyFromMISView(){
		...
		misReplyDetails.setScreenName("ProcessLevel1");
