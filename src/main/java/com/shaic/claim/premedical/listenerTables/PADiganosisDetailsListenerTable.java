...
public class PADiganosisDetailsListenerTable extends ViewComponent {
	...
	private void addListener() {
		btnAdd.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				pedValidationTableDTO.setSublimitApplicable(selectValue);
				pedValidationTableDTO.setConsiderForPayment(selectValue);
...
//			}
			/*R20181279 - Removed ICD Code & Block
				...
				/*if (pedValidation.getIcdBlock() != null) {
					addICDCode(pedValidation.getIcdBlock().getId(), box,
				...
				if (pedValidation.getIcdCode() != null && (presenterString.equalsIgnoreCase("premedicalEnhancement") || bean.getIsBack()
						|| (presenterString.equalsIgnoreCase("PA_MEDICAL_APPROVAL_CLAIM_REQUEST_DATA_EXTRACTION") && !(bean.getHospitalizaionFlag())))) {
...
//			dummyField.setValue(value.getValue());
	...
	public void addICDBlock(Long icdChpterComboKey, ComboBox icdBlockCombo,
		...
		if ("premedicalPreauth".equalsIgnoreCase(this.presenterString)) {
			...
			fireViewEvent(ClaimRequestDataExtractionPagePresenter.GET_ICD_BLOCK,
			icdChpterComboKey);
		...
		for (DiagnosisDetailsTableDTO bean : itemIds) {
			...
			if (validate.size() > 0) {
				...
				for (ConstraintViolation<DiagnosisDetailsTableDTO> constraintViolation : validate) {
					if(constraintViolation.getRootBean() != null && presenterString.equalsIgnoreCase(SHAConstants.PA_MEDICAL_APPROVAL_CLAIM_REQUEST_DATA_EXTRACTION)){
					    DiagnosisDetailsTableDTO rootBean = constraintViolation.getRootBean();
					    errorMessages.add(constraintViolation.getMessage());
					}/*else{
						DiagnosisDetailsTableDTO rootBean = constraintViolation.getRootBean();
								if(!(null == rootBean.getSublimitApplicable() || null == rootBean.getConsiderForPayment()))
								{
										errorMessages.add(constraintViolation.getMessage());
										hasError = true;
								}
