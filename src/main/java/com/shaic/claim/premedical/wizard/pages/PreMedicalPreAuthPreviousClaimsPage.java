...
public class PreMedicalPreAuthPreviousClaimsPage extends PreMedicalPreauthPreviousClaimsPageUI implements WizardStep<PreauthDTO> {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1179139380177682273L;
...
//		viewButton = getViewButton();
		...
		if(null != bean.getNewIntimationDTO() &&  null != bean.getClaimDTO().getNewIntimationDto() && 
				null !=bean.getClaimDTO().getNewIntimationDto().getPolicy() && null != bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct() &&
				...
				{
						preauthPreviousClaimsTableInstance.setVisibleColumnForHealth();
				}
		else
		{
			preauthPreviousClaimsTableInstance.setVisibleColumnForHealth();
		...
		if(!(null != bean.getNewIntimationDTO() && null != bean.getNewIntimationDTO().getIsJioPolicy() && bean.getNewIntimationDTO().getIsJioPolicy()) && !(ReferenceTable.getGMCProductList().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))){
			if(this.bean.getPreviousClaimsList() != null && this.bean.getPreviousClaimsList().isEmpty()){
				...
				fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_FOR_POLICY,bean);
			}
		}
				...
				if (viewType.getValue() != null) {
					if (viewType.getValue().equals(POLICY_WISE)) {
						...
						if(null != bean.getNewIntimationDTO() &&  null != bean.getClaimDTO().getNewIntimationDto() && 
								...
								&& !(ReferenceTable.getGPAProducts().containsKey(bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getKey())))
								{
										preauthPreviousClaimsTableObj.setVisibleColumnForHealth();
								}
						else
						{
							preauthPreviousClaimsTableObj.setVisibleColumnForHealth();
						...
						fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_DETAILS, bean,SHAConstants.POLICY_WISE);
					} else if (viewType.getValue().equals(INSURED_WISE)) {
						preauthPreviousClaimsTableObj.setCaption("Insured Wise");
					//	fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_DETAILS, bean.getPolicyDto().getInsuredId());
						fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_DETAILS, bean,SHAConstants.INSURED_WISE);
						...
						//preauthPreviousClaimsTableObj.setTableList(bean.getPreviousClaimsList());
						fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_DETAILS, bean,SHAConstants.RISK_WISE);
					}
				}
...
//						preauthPreviousClaimsTableObj.setCaption("Insured Wise");
//					//	fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_DETAILS, bean.getPolicyDto().getInsuredId());
//						fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_PREVIOUS_CLAIM_DETAILS, bean);
//						//fireViewEvent(PreauthWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean.getPolicyDto().getInsuredId());
//					} else if (viewType.getValue().equals(RISK_WISE)) {
//						preauthPreviousClaimsTableObj.setCaption("Risk Wise");
//						preauthPreviousClaimsTableObj.setTableList(bean.getPreviousClaimsList());
...
//			});
		...
		cmbRelapseOfIllness.addValueChangeListener(new ValueChangeListener() {
			...
			@Override
			public void valueChange(ValueChangeEvent event) {
				fireViewEvent(PreMedicalPreauthWizardPresenter.PREMEDICAL_RELAPSE_OF_ILLNESS_CHANGED, bean, (SelectValue)event.getProperty().getValue());
			}
		});
	}
	...
	public void setClearTableObj(){
		...
		if(rightFLayout!=null){
			rightFLayout.removeAllComponents();
		}
