...
public class BancsSearchPEDRequestApproveViewImpl  extends AbstractMVPView implements BancsSearchPEDRequestApproveView, Searchable  {
	

	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private BancsSearchPEDRequestApproveForm searchForm;
	
	@Inject
	private BancsSearchPEDRequestApproveTable searchResultTable;
...
resetView();
	...
	public void list(Page<BancsSearchPEDRequestApproveTableDTO> tableRows) {
		...
		{	
			...
			for (BancsSearchPEDRequestApproveTableDTO tableDto : tableRows.getPageItems()) {
		...
		else
		{
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.PROCESS_PED_QUERY_REQUEST, null);
	...
	public void doSearch() {
		BancsSearchPEDRequestApproveFormDTO searchDTO = searchForm.getSearchDTO();
		...
		fireViewEvent(BancsSearchPEDRequestApprovePresenter.BANCS_SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof BancsSearchPEDRequestApproveTable)
			{
				((BancsSearchPEDRequestApproveTable) comp).removeRow();
			}
		}
	}
