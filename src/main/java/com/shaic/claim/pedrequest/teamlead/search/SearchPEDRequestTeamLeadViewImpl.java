...
public class SearchPEDRequestTeamLeadViewImpl  extends AbstractMVPView implements SearchPEDRequestTeamLeadView, Searchable  {
	

	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private SearchPEDRequestTeamLeadForm searchForm;
	
	@Inject
	private SearchPEDRequestTeamLeadTable searchResultTable;
...
resetView();
	...
	public void list(Page<SearchPEDRequestApproveTableDTO> tableRows) {
		...
		else
		{
			...
			Button homeButton = new Button("Process PED Request Home");
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.PROCESS_PED_REQUEST_TL_APPROVE, null);
	...
	public void doSearch() {
		...
		fireViewEvent(SearchPEDRequestTeamLeadPresenter.PED_TL_SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchPEDRequestTeamLeadTable)
			{
				((SearchPEDRequestTeamLeadTable) comp).removeRow();
			}
		}
	}
