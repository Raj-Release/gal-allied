...
@ViewInterface(ProcessRejectionView.class)
public class ProcessRejectionPresenter extends AbstractMVPPresenter<ProcessRejectionView> {
	...
	public static final String CONFIRM_BUTTON="Confirm Rejection Button";
	public static final String WAIVE_BUTTON="Cancel Rejection Button";
	public static final String SET_DATA="Bindind data to field";
	public static final String SUBMIT_DATA="submit data in process Rejection";
	...
	public static final String VALIDATE_PROCESS_REJECTION_USER_RRC_REQUEST = "process_rejection_user_rrc_request";
	
	public static final String PROCESS_REJECTION_LOAD_RRC_REQUEST_DROP_DOWN_VALUES = "process_rejection_load_rrc_request_drop_down_values";
	
	public static final String PROCESS_REJECTION_SAVE_RRC_REQUEST_VALUES = "process_rejection_save_rrc_request_values";

	public static final String PROCESS_REJECTION_LOAD_RRC_REQUEST_SUB_CATEGORY_VALUES = "process_rejection_load_rrc_request_sub_category_values";

	public static final String PROCESS_REJECTION_LOAD_RRC_REQUEST_SOURCE_VALUES = "process_rejection_load_rrc_request_source_values";
	...
	public void generateFieldsBasedOnEnhancementChange(@Observes @CDIEvent(CONFIRM_BUTTON) final ParameterDTO parameters)
	{
		SearchProcessRejectionTableDTO searchDTO = (SearchProcessRejectionTableDTO) parameters.getPrimaryParameter();
		
		BeanItemContainer<SelectValue> rejectionCategory=masterService.getRevisedRejectionCategoryByValue(searchDTO.getIntimationDTO().getPolicy().getProduct().getKey());  //masterService.getRejectionCategoryByValue()
		
		if(searchDTO.getIntimationDTO().getPolicy().getPolicyPlan().equalsIgnoreCase(SHAConstants.POLICY_COVID_GRP_PLAN_LUMPSUM)){
			rejectionCategory = masterService.getRevisedRejectionCategoryByValueForCoron(searchDTO.getIntimationDTO().getPolicy().getProduct().getKey() ,searchDTO.getIntimationDTO().getPolicy().getPolicyPlan());
		}else if(searchDTO.getIntimationDTO().getPolicy().getPolicyPlan().equalsIgnoreCase(SHAConstants.POLICY_COVID_GRP_PLAN_INDEMNITY)){
			rejectionCategory= masterService.getRevisedRejectionCategoryByValueForCoron(searchDTO.getIntimationDTO().getPolicy().getProduct().getKey(),searchDTO.getIntimationDTO().getPolicy().getPolicyPlan()); 
		}else{
			rejectionCategory=masterService.getRevisedRejectionCategoryByValue(searchDTO.getIntimationDTO().getPolicy().getProduct().getKey());
		}
	...
	public void setReferenceData(@Observes @CDIEvent(ProcessRejectionPresenter.SET_DATA)final ParameterDTO parameters){
...
//		bean.setKey(27l);
	...
	public void generateFieldsBasedOnCancel(@Observes @CDIEvent(WAIVE_BUTTON) final ParameterDTO parameters)
	{
	...
	public void saveData(@Observes @CDIEvent(ProcessRejectionPresenter.SUBMIT_DATA)final ParameterDTO parameters){
...
//			view.savedResult();	
			if(outCome.equalsIgnoreCase(SHAConstants.OUTCOME_FLP_NON_MED_CONFIRM_REJECTION)){
...
//						masterService);
	...
	public void setUpSubCategoryValues(
			@Observes @CDIEvent(PROCESS_REJECTION_LOAD_RRC_REQUEST_SUB_CATEGORY_VALUES) final ParameterDTO parameters) {
	...
	public void setUpsourceValues(
			@Observes @CDIEvent(PROCESS_REJECTION_LOAD_RRC_REQUEST_SOURCE_VALUES) final ParameterDTO parameters) {
