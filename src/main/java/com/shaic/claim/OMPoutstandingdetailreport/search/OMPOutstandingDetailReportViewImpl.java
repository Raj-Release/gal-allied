...
public class OMPOutstandingDetailReportViewImpl extends AbstractMVPView implements OMPOutstandingDetailReportView{
	@Inject
	private OMPOutstandingDetailReportUI searchForm;
	private OMPOutstandingDetailReportFormDto searchDto;
	@Inject
	private OMPOutstandingDetailReportDetailTable searchResultTable;
...
private VerticalSplitPanel mainPanel;
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof OMPOutstandingDetailReportDetailTable)
			{
				((OMPOutstandingDetailReportDetailTable) comp).removeRow();
			}
		}
	...
	public void doSearch() {
		OMPOutstandingDetailReportFormDto searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		fireViewEvent(OMPOutstandingDetailReportPresenter.SUBMIT_SEARCH, searchDTO,userName,passWord);
