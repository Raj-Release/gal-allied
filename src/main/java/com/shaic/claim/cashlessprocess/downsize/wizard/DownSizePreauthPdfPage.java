...
public class DownSizePreauthPdfPage extends ViewComponent implements WizardStep<PreauthDTO> {
	...
	public Component getContent() {
       ...
       // panel.setSizeFull();
		...
		if(this.bean.getPreauthDataExtractionDetails().getTotalApprAmt() != null){
			this.bean.setPreviousPreauthPayableAmount(this.bean.getPreauthDataExtractionDetails().getTotalApprAmt().intValue());
		}
		...
		String templateName = "PreauthDownSizeLetter";  // OLD Template
		
		if(!ReferenceTable.getGMCProductList().containsKey(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())) {
			templateName = "PreauthDownSizeLetter_Non_GMC";			// R1313 03-01-2019  // R2019063  New Template
		}
