...
public class SearchFlagReconsiderationReqViewImpl extends AbstractMVPView implements SearchFlagReconsiderationReqView, Searchable {
	
	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private SearchFlagReconsiderationReqForm searchForm;
	
	@Inject
	private SearchFlagReconsiderationReqTable searchResultTable;	
	...
	public void list(Page<SearchFlagReconsiderationReqTableDTO> tableRows) {
		...
		{	
			...
			for (SearchFlagReconsiderationReqTableDTO tableDto : tableRows.getPageItems()) {
	...
	public void doSearch() {
		SearchFlagReconsiderationReqFormDTO searchDTO = searchForm.getSearchDTO();
		...
		fireViewEvent(SearchFlagReconsiderationReqPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchFlagReconsiderationReqTable)
			{
				((SearchFlagReconsiderationReqTable) comp).removeRow();
			}
		}
	}
