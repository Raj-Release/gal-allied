...
public class DownsizePreauthRequestPdfPage extends ViewComponent implements WizardStep<PreauthDTO> {
			...
			for(PreviousPreAuthTableDTO prevPreauthDto : this.bean.getPreviousPreauthTableDTO()){
				if(prevPreauthDto.getApprovedAmt() != null && !("0.0").equalsIgnoreCase(prevPreauthDto.getApprovedAmt())) {
					...
					if(prevApprovDetails.equalsIgnoreCase("")) {
						if(bean.getPolicyInstalmentFlag() != null && bean.getPolicyInstalmentFlag().equals(SHAConstants.YES_FLAG)){
							prevApprovDetails = (prevPreauthDto.getApprovedAmtAftPremiumDeduction() != null && prevPreauthDto.getModifiedDate() != null) ? "Rs."+ (int)Double.parseDouble(prevPreauthDto.getApprovedAmtAftPremiumDeduction()) + "/- on " + (new SimpleDateFormat("dd-MMM-yy").format(prevPreauthDto.getModifiedDate())).toUpperCase() : "";	
						}
						else if(ReferenceTable.STAR_UNIQUE_PRODUCT_KEY.equals(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())){
							prevApprovDetails = (prevPreauthDto.getApprovedAmtAftPremiumDeduction() != null && prevPreauthDto.getModifiedDate() != null) ? "Rs."+ (int)Double.parseDouble(prevPreauthDto.getApprovedAmtAftPremiumDeduction()) + "/- on " + (new SimpleDateFormat("dd-MMM-yy").format(prevPreauthDto.getModifiedDate())).toUpperCase() : "";
						}
						else{
							prevApprovDetails = (prevPreauthDto.getApprovedAmt() != null && prevPreauthDto.getModifiedDate() != null) ? "Rs."+ (int)Double.parseDouble(prevPreauthDto.getApprovedAmt()) + "/- on " + (new SimpleDateFormat("dd-MMM-yy").format(prevPreauthDto.getModifiedDate())).toUpperCase() : "";		
						}
					}
					else{
						if(bean.getPolicyInstalmentFlag() != null && bean.getPolicyInstalmentFlag().equals(SHAConstants.YES_FLAG)){
							prevApprovDetails = prevApprovDetails + ", and " +( (prevPreauthDto.getApprovedAmtAftPremiumDeduction() != null && prevPreauthDto.getModifiedDate() != null) ? "Rs."+ (int)Double.parseDouble(prevPreauthDto.getApprovedAmtAftPremiumDeduction()) + "/- on " + (new SimpleDateFormat("dd-MMM-yy").format(prevPreauthDto.getModifiedDate())) .toUpperCase(): "");	
						}
						else if(ReferenceTable.STAR_UNIQUE_PRODUCT_KEY.equals(bean.getNewIntimationDTO().getPolicy().getProduct().getKey())){
							prevApprovDetails = prevApprovDetails + ", and " +( (prevPreauthDto.getApprovedAmtAftPremiumDeduction() != null && prevPreauthDto.getModifiedDate() != null) ? "Rs."+ (int)Double.parseDouble(prevPreauthDto.getApprovedAmtAftPremiumDeduction()) + "/- on " + (new SimpleDateFormat("dd-MMM-yy").format(prevPreauthDto.getModifiedDate())) .toUpperCase(): "");
						}
						else{
							prevApprovDetails = prevApprovDetails + ", and " +( (prevPreauthDto.getApprovedAmt() != null && prevPreauthDto.getModifiedDate() != null) ? "Rs."+ (int)Double.parseDouble(prevPreauthDto.getApprovedAmt()) + "/- on " + (new SimpleDateFormat("dd-MMM-yy").format(prevPreauthDto.getModifiedDate())).toUpperCase() : "");
						}
					}
				}
			}
