...
@Stateless
public class SearchSettlementPullBackService extends AbstractDAO<Intimation> {
	Map<Long, Object> workFlowMap= new WeakHashMap<Long, Object>();
	
	public SearchSettlementPullBackService(){
		
	}
	
	
	public  Page<SearchSettlementPullBackDTO> searchForSettlementPullBack(
			SearchSettlementPullBackFormDTO formDTO,
			String userName, String passWord) {
		Page<SearchSettlementPullBackDTO> page = new Page<SearchSettlementPullBackDTO>();
...
//		taskProcedure = dbCalculationService.revisedGetTaskProcedure(setMapValues);	
		...
		List<SearchSettlementPullBackDTO> dtoList = new ArrayList<SearchSettlementPullBackDTO>();
		if(intimationNo != null && taskProcedure != null && !taskProcedure.isEmpty()) { // extra Condtion added IMSSUPPOR-28837 Qry Tn
			/*Below Code Commented and add New Qry For IMSSUPPOR-28837 - Qry Tn*/
				...
				if(claimforIntimation != null) {
					...
					if(rodByClaimKey != null && !rodByClaimKey.isEmpty()) {
						try {
							...
							//GetAllHTInfoClient allCompletedTask = BPMClientContext.getAllCompletedTask();
							//com.oracle.xmlns.bpm.bpmobject.claimreimbcabusinessobjects.payloadbo.PayloadBOType payloadBOType = new com.oracle.xmlns.bpm.bpmobject.claimreimbcabusinessobjects.payloadbo.PayloadBOType();
							//PagedTaskList pageList = allCompletedTask.getTasks(BPMClientContext.BPMN_TASK_USER, "previous", "completed", "Process ClaimFinancials->IntimationNumber :" +intimationByNo.getIntimationId(), "processClaimFinancials", null, payloadBOType );
							//List<HumanTask> humanTasks = pageList.getHumanTasks();
							...
							for (Reimbursement reimbursement : rodByClaimKey) {
								System.out.println("---Start time of UNDO FA getPaymentDetailsByRodNumber()---"+System.currentTimeMillis()+" "+intimationByNo.getIntimationId());
								
								ClaimPayment paymentDetailsByRodNumber = getPaymentDetailsByRodNumber(reimbursement.getRodNumber());
								
								System.out.println("---End time of UNDO FA getPaymentDetailsByRodNumber()---"+System.currentTimeMillis()+" "+intimationByNo.getIntimationId());
								Boolean faApprovedAndNotLotCreated = SHAUtils.isFAApprovedAndNotLotCreated(reimbursement, paymentDetailsByRodNumber);
								if(faApprovedAndNotLotCreated) {
									String message = "";
									if(reimbursement.getDocAcknowLedgement() != null && ((reimbursement.getDocAcknowLedgement().getHospitalisationFlag() != null && reimbursement.getDocAcknowLedgement().getHospitalisationFlag().equalsIgnoreCase("Y") || reimbursement.getDocAcknowLedgement().getPartialHospitalisationFlag() != null && reimbursement.getDocAcknowLedgement().getPartialHospitalisationFlag().equalsIgnoreCase("Y")))) {
										for (Reimbursement otherROD : rodByClaimKey) {
											if(!otherROD.getKey().equals(reimbursement.getKey())) {
												if(otherROD.getDocAcknowLedgement() != null && ((otherROD.getDocAcknowLedgement().getPreHospitalisationFlag() != null && otherROD.getDocAcknowLedgement().getPreHospitalisationFlag().equalsIgnoreCase("Y") || otherROD.getDocAcknowLedgement().getPostHospitalisationFlag() != null && otherROD.getDocAcknowLedgement().getPostHospitalisationFlag().equalsIgnoreCase("Y")))) {
													ClaimPayment otherRODPayment = getPaymentDetailsByRodNumber(reimbursement.getRodNumber());
													Boolean otherRODFAApprovedOrNot = SHAUtils.isFAApprovedAndNotLotCreated(otherROD, otherRODPayment);
													if(otherRODFAApprovedOrNot) {
														message = "There is Pre/Post hospitalization is also FA approved. Please do pullback for Pre/Post hospitalization";
													} else if(SHAUtils.isFAApprovedAndLotCreatedForOtherRODs(otherROD, otherRODPayment)) {
														message = "Pullback is not possibe for this record since there is a Pre/Post hospitalization already FA approved and lot created";
													}
												}
											}
										}
										...
										if(null != workflowKey) {
											SearchSettlementPullBackDTO filledDTO = getFilledDTO(intimationByNo, claimforIntimation, reimbursement, userName, passWord, true);
											//filledDTO.setHumanTask(humanTask);
											filledDTO.setDbOutArray(workflowKey);
											filledDTO.setMessage(message);
											filledDTO.setClaimPayment(paymentDetailsByRodNumber);
...
//						dtoList.add(getFilledDTO(intimationByNo, claimforIntimation, null, userName, passWord, false));
	...
	protected SearchSettlementPullBackDTO getFilledDTO(Intimation intimation, Claim claim, Reimbursement reimbursement,  String userName, String password, Boolean isAllowedToPullBack) {
		SearchSettlementPullBackDTO dto = new SearchSettlementPullBackDTO();
		System.out.println("---Start time of UNDO FA getIntimationDto()---"+System.currentTimeMillis()+" "+intimation.getIntimationId());
		NewIntimationDto intimationDto = getIntimationDto(intimation);
		System.out.println("---End time of UNDO FA getIntimationDto()---"+System.currentTimeMillis()+" "+intimation.getIntimationId());
...
//		dto.setPolicyDivCode(intimation.getPolicy().get);
	...
	public NewIntimationDto getIntimationDto(Intimation intimation) {
		...
		if (null != intimation) {
			...
			System.out.println("---Start time of UNDO FA getNewIntimationDto()---"+System.currentTimeMillis()+" "+intimation.getIntimationId());
			intimationToRegister = newIntimationMapper
					.getNewIntimationDto(intimation);
			System.out.println("---End time of UNDO FA getNewIntimationDto()---"+System.currentTimeMillis()+" "+intimation.getIntimationId());
			...
			if (null != intimation.getPolicy()) {
				if(null == intimation.getPolicy().getInsured() || intimation.getPolicy().getInsured().isEmpty()){
					System.out.println("---Start time of UNDO FA getInsuredListByPolicyNumber()---"+System.currentTimeMillis()+" "+intimation.getIntimationId());
					List<Insured> insuredList = getInsuredListByPolicyNumber(intimation.getPolicy().getPolicyNumber());
					System.out.println("---End time of UNDO FA getInsuredListByPolicyNumber()---"+System.currentTimeMillis()+" "+intimation.getIntimationId());
