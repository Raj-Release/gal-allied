...
@Stateless
public class SettlementPullBackService {
	...
	public DocumentDetails getQueryDocumentDetailsDataByRod(String rodNumber,String docType,String docTypeForScrc,String docSource)
	{
		Query query = entityManager.createNamedQuery("DocumentDetails.findQueryByRodNoDesc");
	...
	public void submitSettlementPullBack(SearchSettlementPullBackDTO dto, Reimbursement reimbursement) {
		try {
			...
			if(docAcknowLedgement != null && docAcknowLedgement.getDocumentReceivedFromId() != null && reimbursement.getReconsiderationRequest() != null && reimbursement.getReconsiderationRequest().equalsIgnoreCase(SHAConstants.YES_FLAG) && (docAcknowLedgement.getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_INSURED) || docAcknowLedgement.getDocumentReceivedFromId().getKey().equals(ReferenceTable.RECEIVED_FROM_HOSPITAL))) {
				...
				List<Reimbursement> reimbursementList = getReimbursementObject(reimbursement.getRodNumber());
                AcknowledgeDocumentReceivedMapper acknowledgeDocumentReceivedMapper =AcknowledgeDocumentReceivedMapper.getInstance();
                List<ReconsiderRODRequestTableDTO> reconreimbursementList = acknowledgeDocumentReceivedMapper.getReimbursementDetails(reimbursementList);
                
	              if(reconreimbursementList!=null && !reconreimbursementList.isEmpty()){
	                for(ReconsiderRODRequestTableDTO reconreimbursement:reconreimbursementList){
	                ReimbursementRejection reimbRejection = getReimbRejectRecord(reconreimbursement.getKey());
	                ...
	                if(reimbRejection.getStatus().getKey().equals(ReferenceTable.PROCESS_CLAIM_FINANCIAL_APPROVE_REJECT_STATUS) || reimbRejection.getStatus().getKey().equals(ReferenceTable.PROCESS_CLAIM_REQUEST_APPROVE_REJECT_STATUS)
	                                        ...
	                                        || reimbRejection.getStatus().getKey().equals(ReferenceTable.PAYMENT_REJECTED))
	                                                {
	                        reconreimbursementList.get(0).setIsRejectReconsidered(true);
	                                                }
	                        else
	                                                {
	                        reconreimbursementList.get(0).setIsRejectReconsidered(false);
	                                                }
	                                        }
	                                }
	                }
	            ...
	            //IMSSUPPOR-28853
	              if(isSettledPaymentAvailable(reimbursement.getRodNumber())){
	            	  reconreimbursementList.get(0).setIsRejectReconsidered(false);
	              }
	              
                if(reconreimbursementList != null && !reconreimbursementList.isEmpty() && reconreimbursementList.get(0).getIsRejectReconsidered() != null && reconreimbursementList.get(0).getIsRejectReconsidered().equals(Boolean.TRUE)){
...
//                                Integer amt = hospCalc.getAmountAlreadyPaidAmt() - (hospCalc.getBalanceToBePaidAmt() != null ? hospCalc.getBalanceToBePaidAmt() : 0);
                                hospCalc.setAmountAlreadyPaidAmt(0);
...
//                                Integer amt = preHospCalc.getAmountAlreadyPaidAmt() - (preHospCalc.getBalanceToBePaidAmt() != null ? preHospCalc.getBalanceToBePaidAmt() : 0);
                                preHospCalc.setAmountAlreadyPaidAmt(0);
...
//                                Integer amt = postHospCalc.getAmountAlreadyPaidAmt() - (postHospCalc.getBalanceToBePaidAmt() != null ? postHospCalc.getBalanceToBePaidAmt() : 0);
                                postHospCalc.setAmountAlreadyPaidAmt(0);
			...
			if(dto.getClaimPayment() != null) {
				...
				claimPayment.setModifiedBy(strUserName);
			...
			DocumentDetails queryDocumentDetailsDataByRod = getQueryDocumentDetailsDataByRod(reimbursement.getRodNumber(), SHAConstants.BILL_SUMMARY_OTHER_PRODUCTS,SHAConstants.BILLASSESSMENTSHEETSCRC,SHAConstants.FINANCIAL_APPROVER);
			...
			//IMSSUPPOR-29737
			if(queryDocumentDetailsDataByRod != null){
				queryDocumentDetailsDataByRod.setDeletedFlag("Y");
				entityManager.merge(queryDocumentDetailsDataByRod);
				entityManager.flush();
				entityManager.clear();
			}
	...
	/*private void submitBPMTaskForPullBack(SearchSettlementPullBackDTO dto, Reimbursement reimbursement) {
		...
		try {
			if(humanTask != null) {
				...
				if(paymentInfo != null && paymentInfo.getClaimedAmount() == null){
...
//							.getTreatmentType().getValue());
	...
	*/
	...
	private void submitPullBackTaskToDB(SearchSettlementPullBackDTO dto)
	{
	 ...
	 public List<Reimbursement> getReimbursementObject(String rodNo)
     {
             ...
             {
//                     entityManager.refresh(reimbursementObjectList);
