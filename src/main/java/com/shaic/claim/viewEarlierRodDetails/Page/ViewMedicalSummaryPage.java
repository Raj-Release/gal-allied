	...
	public void init(Long rodKey) {
		...
		if (null != reimbursement) {
			...
			if(reimbursement !=null && reimbursement.getRoomCategory()!=null && reimbursement.getRoomCategory().getValue()!=null
				...
				System.out.println(String.format("Ventilator Support value in View Details in medical summary: [%s]", reimbursement.getVentilatorSupport()));
...
//		viewDiagnosisTableObj = viewDiagnosisTableInstance.get();
		...
		FormLayout firstForm = new FormLayout(txtStatus, txtApprovedAmount,
				...
				ReasonForChangeAdmission, txtRoomCategory,ventilatorSupportRadio,txtTypeOfAdmission, txtSpecify,
				txtDateOfDischarge, txtSystemMedicine, txtHospitalisationDueTo,
				txtDateOfInjury, medicoLegalCase, reportedToPolice,
				txtTreatementType,txtDecisionChangeReason,txtDecisionChangeRemarks);
...
//		reimbursement.getClaim().getIntimation().getIntimationId()
		...
		VerticalLayout verticalMain = new VerticalLayout(buttonHor,
				...
				/*investigationHor ,*/specialistHLayout, dummyHor, /*dummyver,*/cancelledFvrInvsQueryObj);
...
//			PreMedicalMapper.getAllMapValues();
	...
	public void showScoringView(){
		...
		scoringAndPPDetailView.setIntimationNumber(reimbursement.getClaim().getIntimation().getIntimationId());
