...
public class SearchConvertReimbursementTable extends
...
//		SHAUtils.setActiveOrDeactiveClaim(t.geiftUsername(),t.getPassword(),t.getTaskNumber(),session);
	if(t.getKey() != null){
		...
		if(ackDocReceivedService.getDBTaskForPreauth(claimByClaimKey.getIntimation(), SHAConstants.REIM_CONVERSION_PROCESS_CURRENT_QUEUE)){
			...
			fireViewEvent(MenuPresenter.SHOW_CONVERT_REIMBURSEMENT, t);
		}
	}else{
		fireViewEvent(MenuPresenter.SHOW_CONVERT_REIMBURSEMENT, t);
	}
