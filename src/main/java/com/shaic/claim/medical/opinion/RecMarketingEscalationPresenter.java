...
@ViewInterface(RecMarketingEscalationView.class)
public class RecMarketingEscalationPresenter extends
AbstractMVPPresenter<RecMarketingEscalationView> {
	...
	public static final String SUBMIT_DATA = "submit record marketing escalation";
	...
	{
		RecordMarkEscDTO bean = (RecordMarkEscDTO)parameters.getPrimaryParameter();
		
		Claim claim = claimService.getClaimByClaimKey(bean.getClaimDTO().getKey());
		
		recMarkEscService.sumbitRecMarkEsc(bean);
