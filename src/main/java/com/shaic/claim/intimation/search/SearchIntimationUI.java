...
public class SearchIntimationUI extends ViewComponent {
...
//	private PagedTable searchIntimationTable;
	...
	@Inject
	SearchIntimationDetailTable searchResultTable;
...
//		searchResultTable.getPageable().setPageNumber(1);
	...
	public void buildSearchIntimationTable(
			Page<NewIntimationDto> newIntimationDtoPagedContainer,SearchIntimationDetailTable intimationDetailTable) {
		...
		if (newIntimationDtoPagedContainer != null) {
			...
			if (selectedIntimStatus != null){
					if(StringUtils.contains(selectedIntimStatus.getValue(),
						...
						searchResultTable.addClaimNumberColum();
//						searchResultTable.addViewIntimationDetailsColumn();
					}
					else if(!StringUtils.contains(selectedIntimStatus.getValue(), "Submitted")){
						searchResultTable.setDraftTableHeader();
						searchResultTable.addEditColumn();				
					}
			}	
...
//			mainPanel.setSecondComponent(searchTableLayout);
	...
	private Panel buildSearchPanel() {
		...
		searchPanel.setCaption("Intimation Search");
...
//		}
		...
		if (mainPanel.getSecondComponent() != null) {
			...
			if(searchResultTable != null){
				searchResultTable.getPageable().setPageNumber(0);
				...
				if(isValid()){
					...
					if (cmbIntimationStatus.getValue() != null) {
						...
						if (cmbIntimationStatus.getValue() != null
							...
							filters.put("pageable", searchResultTable.getPageable());
...
//		 gridLayout_1.addComponent(searchButton, 0, 0);
		...
		resetButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(SearchIntimationPresenter.RESET_SEARCH_VIEW, null);
			}
		});
...
//								((ComboBox) field).setValue(null);
							...
							if (field instanceof ComboBox) {
//								((ComboBox) field).setValue(null);
								if(field.getId() != null && field.getId().equalsIgnoreCase("year")){
								field.setEnabled(true);
