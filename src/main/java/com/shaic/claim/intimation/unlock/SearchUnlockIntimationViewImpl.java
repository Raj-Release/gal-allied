...
public class SearchUnlockIntimationViewImpl extends AbstractMVPView implements SearchUnlockIntimationView {
	...
	@Inject
	private SearchUnlockIntimationForm searchForm;
	
	@Inject
	private SearchUnlockIntimationTable searchResultTable;
	...
	public void doSearch() {
		...
		if(searchForm.validatePage()){
			SearchUnlockIntimationFormDTO searchDTO = searchForm.getSearchDTO();
			Pageable pageable = searchResultTable.getPageable();
			searchDTO.setPageable(pageable);
			String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
			String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
			fireViewEvent(SearchUnlockIntimationPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
		} else {
			
			showErrorMessage("Please Enter the last 7 digits of the Intimation Number");
			resetView();
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof SearchUnlockIntimationTable)
			{
				((SearchUnlockIntimationTable) comp).removeRow();
			}
		}
	}
	...
	@Override
	public void list(Page<SearchUnlockIntimationTableDTO> tableRows) {
