...
//		AuditDetails auditDetails = masterService.getCVCAuditActionByintimationKey(tableDto.getIntimationKey());
		...
		if(auditCategory!=null && !auditCategory.isEmpty()){
			for (AuditCategory AuditCategory : auditCategory) {
				...
				if(auditMasCategory != null){
					cmbCategoryOfErrortemp.setValue(auditMasCategory.getCategory());
					cmbCategoryContainer.addBean(cmbCategoryOfErrortemp);
				} else {
					MasPostClaimCVCCategory postClaimauditMasCategory = masterService.getPostClaimCVCMasCategoryValue(AuditCategory.getAuditCategory());
					cmbCategoryOfErrortemp.setValue(postClaimauditMasCategory.getCategory());
					cmbCategoryContainer.addBean(cmbCategoryOfErrortemp);
					isPostClaim = true;
				}
			}
		...
		BeanItemContainer<SelectValue> ErrorValueContainer = null;
		if(!isPostClaim){
			ErrorValueContainer = masterService.getCVCErrorCategoryByMaster();
		}else {
			ErrorValueContainer = masterService.getPostClaimCVCErrorCategoryByMaster();
		}
