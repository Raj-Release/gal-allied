...
public class CVCAuditQryApprovalPageUI extends ViewComponent {
...
//	private TextArea txtAuditQryRemarks;
...
//	FormLayout cmbRightFLayout1;
	...
	private Panel qryPanel;
	
	private Panel adQryPanel;
	...
	public void setCompleteLayout() {
		...
		clsQryTable.init(SHAConstants.AUDIT_QRY_APPROVAL_SCREEN,bean);
		
		billingFaQryTable = billingFaQryTableInstance.get();
		billingFaQryTable.init(SHAConstants.AUDIT_QRY_APPROVAL_SCREEN, bean);
		
		medicalQryTable = medicalQryTableInstance.get();
		medicalQryTable.init(SHAConstants.AUDIT_QRY_APPROVAL_SCREEN, bean);
...
//		cmbProcessor.setNullSelectionAllowed(Boolean.FALSE);
		...
		fireViewEvent(CVCAuditQryApprovalPagePresenter.LOAD_CVC_AUDIT_ACTION_STATUS_VALUES, bean);
...
//			setRequiredAndValidation(txtClsQryRemarks);
		...
		if(!StringUtils.isBlank(bean.getReimbAdQryRemarks())){
			txtReimbQryRemarks.setValue(bean.getReimbAdQryRemarks());
	...
	private void addListener(){
				        ...
				        "No", "Yes", new ConfirmDialog.Listener() {
				            ...
				            public void onClose(ConfirmDialog dialog) {
				                if (!dialog.isConfirmed()) {
				                	fireViewEvent(CVCAuditQryApprovalPagePresenter.CANCEL_EVENT_AUDIT_APPROVAL, bean);
				                	releaseHumanTask();
				                	fireViewEvent(MenuItemBean.CVC_AUDIT_QRY_APPRVOAL, true);
				                } else {
				                    dialog.close();
				                }
				            }
		...
		submitBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(validatePage())
				{
					...
					fireViewEvent(CVCAuditQryApprovalPagePresenter.SUBMIT_EVENT_AUDIT_APPROVAL, bean);
				}
		...
		cmbTeam.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				//CR2019181 AUDIT QRY REMARKS LAYOUT
...
public boolean validatePage() {
		...
		Boolean isQueryNotAcceptedOnMA = true;
		Boolean isQueryNotAcceptedOnCashless = true;
		Boolean isQueryNotAcceptedOnFA = true;
		...
		if(medicalQryTable != null && medicalQryTable.getValues() != null  && !medicalQryTable.getValues().isEmpty()) {
			 isQueryNotAcceptedOnMA = medicalQryTable.isQueryAcceptedOrNotMA();
		}
		if(clsQryTable != null && clsQryTable.getValues() != null  && !clsQryTable.getValues().isEmpty()) {
			isQueryNotAcceptedOnCashless = clsQryTable.isQueryAcceptedOrNotCashless();
		}
		if(billingFaQryTable != null && billingFaQryTable.getValues() != null  && !billingFaQryTable.getValues().isEmpty()) {
			isQueryNotAcceptedOnFA = billingFaQryTable.isQueryAcceptedOrNotFA();
		}
		if(isQueryNotAcceptedOnMA && isQueryNotAcceptedOnCashless && isQueryNotAcceptedOnFA){
			if(cmbAuditRemediationStatus.getValue() != null
					&&  !cmbAuditRemediationStatus.getValue().toString().equalsIgnoreCase(SHAConstants.COMPLETED_STRING)){
				hasError = true;
				eMsg.append("Please Select Audit Remediation Status to Complete</br>");
			}
		}
		//added for jira ID IMSSUPPOR-31174
		if (hasError) {
 ...
 * */
	...
	public void generateQryOutcomeLayout() {
		...
		HorizontalLayout completQryHLayout = new HorizontalLayout();
		...
		cmbCompletedReason.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				SelectValue selectedCompletedReason = (SelectValue)((ComboBox)event.getProperty()).getValue();
				bean.setCompletedReason(selectedCompletedReason.getValue());
		}
			}
		});
		...
		txtCompletedRemarks = new TextArea("CompletedRemarks");
		...
		if(multiSelect != null && !multiSelect.isEmpty()) {
			...
			for (Object obj : multiSelect) {				
				...
				if(teamSelect.getValue().equalsIgnoreCase(SHAConstants.AUDIT_TEAM_CASHLESS)) {
					...
					clsQryTable.generateAcceptNotAcceptBtn();
				...
				if(teamSelect.getValue().equalsIgnoreCase(SHAConstants.AUDIT_TEAM_MEDICAL)) {
					...
					medicalQryTable.generateAcceptNotAcceptBtn();
				...
				if(teamSelect.getValue().equalsIgnoreCase(SHAConstants.AUDIT_TEAM_BILLING) || teamSelect.getValue().equalsIgnoreCase(SHAConstants.AUDIT_TEAM_FINANCIAL)){
					...
					billingFaQryTable.generateAcceptNotAcceptBtn();
