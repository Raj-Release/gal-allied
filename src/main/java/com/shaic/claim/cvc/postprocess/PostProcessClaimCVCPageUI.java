...
import com.shaic.claim.cvc.SearchCVCForm;
import com.shaic.claim.cvc.SearchCVCTableDTO;
import com.shaic.claim.cvc.postprocess.PostProcessClaimCVCPresenter;
...
public class PostProcessClaimCVCPageUI extends ViewComponent {
...
//		cmbProcessor.setNullSelectionAllowed(Boolean.FALSE);
		...
		fireViewEvent(PostProcessClaimCVCPresenter.LOAD_POST_PROCESS_CVC_STATUS_VALUES, bean);
...
//				                	fireViewEvent(CVCPagePresenter.CANCEL_EVENT, bean);
		...
		submitBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(validatePage())
				{
					fireViewEvent(PostProcessClaimCVCPresenter.POST_PROCESS_CVC_SUBMIT_EVENT, bean);
				}
...
//						verticalLayout = new VerticalLayout(remarksFLayout,horizontalLayout);
						...
						bean.setQryStatus(SHAConstants.AUDIT_QUERY_COMPLETED);
...
//				txtArea.setReadOnly(true);
	...
	public void loadCVCStatusDropDownValues(BeanItemContainer<SelectValue> statusValueContainer,
		...
		cmbAuditRemediationStatus.setValue(remediationStatusValueContainer.getItemIds().get(0));
		bean.setRemediationStatus(cmbAuditRemediationStatus.getValue().toString());
