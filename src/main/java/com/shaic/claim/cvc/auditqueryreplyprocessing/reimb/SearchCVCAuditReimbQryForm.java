...
public class SearchCVCAuditReimbQryForm extends SearchComponent<SearchCVCAuditClsQryFormDTO>{
	
	private SearchCVCAuditClsQryFormDTO searchDto;
	
	private SearchCVCAuditReimbQryTable searchTable;
	...
	public void init() {
		...
		mainPanel.setCaption("Claim Audit Query (Reimbursement)");
		...
		cmbRole.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(com.vaadin.v7.data.Property.ValueChangeEvent event) {
				if(event.getProperty().getValue() != null) {
					...
					empContainer = masterService.getAuditQryReplyUser(roleSelect.getValue(), SHAConstants.AUDIT_MEDICAL_QRY_REPLY_SCREEN);
...
//		empContainer = masterService.getAuditReplyUser(SHAConstants.CASHLESS.toLowerCase(),SHAConstants.AUDIT_TEAM_MEDICAL.toLowerCase());
	...
	public void setDropDownValues(SearchCVCAuditClsQryFormDTO srchFrmDto) {
		...
		cmbRole.setValue(auditUserRoleContainer.getIdByIndex(0));
