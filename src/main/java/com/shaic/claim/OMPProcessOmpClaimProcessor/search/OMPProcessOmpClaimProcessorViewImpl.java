...
public class OMPProcessOmpClaimProcessorViewImpl extends AbstractMVPView implements OMPProcessOmpClaimProcessorView{
	
	
	@Inject
	private OMPProcessOmpClaimProcessorUI searchForm;
	
	OMPProcessOmpClaimProcessorFormDto searchDto;
	
	@Inject
	private OMPProcessOmpClaimProcessorDetailTable searchResultTable;
...
private VerticalSplitPanel mainPanel;
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof OMPProcessOmpClaimProcessorDetailTable)
			{
				((OMPProcessOmpClaimProcessorDetailTable) comp).removeRow();
			}
		}
	...
	public void doSearch() {
		OMPProcessOmpClaimProcessorFormDto searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		fireViewEvent(OMPProcessOmpClaimProcessorPresenter.SUBMIT_SEARCH, searchDTO,userName,passWord);
