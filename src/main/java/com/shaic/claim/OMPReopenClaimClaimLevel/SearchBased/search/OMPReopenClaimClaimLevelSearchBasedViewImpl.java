...
public class OMPReopenClaimClaimLevelSearchBasedViewImpl extends AbstractMVPView implements OMPReopenClaimClaimLevelSearchBasedView{
	
	@Inject
	private OMPReopenClaimClaimLevelSearchBasedUI searchForm;
	private OMPReopenClaimClaimLevelSearchBasedFormDto searchDto;
	@Inject
	private OMPReopenClaimClaimLevelSearchBasedDetailTable searchResultTable;
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof OMPReopenClaimClaimLevelSearchBasedDetailTable)
			{
				((OMPReopenClaimClaimLevelSearchBasedDetailTable) comp).removeRow();
			}
		}
	...
	public void doSearch() {
		OMPReopenClaimClaimLevelSearchBasedFormDto searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		fireViewEvent(OMPReopenClaimClaimLevelSearchBasedPresenter.SUBMIT_SEARCH, searchDTO,userName,passWord);
