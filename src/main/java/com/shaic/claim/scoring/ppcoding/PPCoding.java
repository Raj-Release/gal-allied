...
@Entity
@Table(name="IMS_CLS_NW_PP_CODING")
@NamedQueries({
	@NamedQuery(name="PPCoding.findAll", query="SELECT m FROM PPCoding m"),
	@NamedQuery(name="PPCoding.findByIntimationKey", query="SELECT m FROM PPCoding m WHERE m.intimationKey = :intimationKey AND m.ppVersion= :ppVersion AND ( m.deleteFlag = 0 OR m.deleteFlag IS NULL) ORDER BY m.ppCode ASC"),
	@NamedQuery(name="PPCoding.findByppstage", query="SELECT m FROM PPCoding m WHERE m.intimationKey = :intimationKey AND m.ppVersion= :ppVersion AND m.deleteFlag = 1 AND m.ppStage = :ppStage ORDER BY m.ppCode ASC")

})
public class PPCoding extends AbstractEntity{

	/**
	 * 
	 */
	private static final long serialVersionUID = -1544178394256314810L;
