 ...
 *
 */
public class DecideOnProcessDraftQueryWizardViewImpl extends AbstractMVPView implements
DecideOnQueryWizard{
	...
	private static final long serialVersionUID = 1L;
		...
		@Inject
		private Instance<DecideOnQueryView> decideOnQueryViewImpl;
		
		
		@Inject
		private ConfirmQueryViewImpl confirmQueryViewImpl;
...
//			HorizontalLayout crmFlaggedLayout = SHAUtils.crmFlaggedLayout(bean.getCrcFlaggedReason(),bean.getCrcFlaggedRemark());
		...
		public void activeStepChanged(WizardStepActivationEvent event) {			
			...
			if(className.contains("DecideOnQueryViewImpl"))	{
//				decideOnQueryViewImpl.get().setUpdatedBean(this.bean);
				((DecideOnQueryViewImpl)event.getActivatedStep()).returnPreviousPage(this.bean);
			}
		}
		...
		public void wizardCompleted(WizardCompletedEvent event) {
			...
			fireViewEvent(DecideOnQueryWizardPresenter.SUBMIT_DRAFT_QUERY, this.bean);
