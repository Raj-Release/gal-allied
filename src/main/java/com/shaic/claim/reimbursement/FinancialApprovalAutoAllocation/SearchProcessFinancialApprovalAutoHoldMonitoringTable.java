...
import com.shaic.claim.reimbursement.commonBillingFA.SearchProcessClaimCommonBillingAndFinancialsHoldMonitoringTable;
...
public class SearchProcessFinancialApprovalAutoHoldMonitoringTable extends GBaseTable<SearchHoldMonitorScreenTableDTO>{

	
	private final Logger log = LoggerFactory.getLogger(SearchProcessFinancialApprovalAutoHoldMonitoringTable.class);
	...
	public void setTableDtoValues(SearchHoldMonitorScreenTableDTO holdMonitorDto){
		...
		SearchProcessClaimFinancialsTableDTO preauthDto = new SearchProcessClaimFinancialsTableDTO();
...
//		preauthDto.setIsAutoAllocationTrue(imsUser);
		...
		if(productKey != null && ! ReferenceTable.getGMCProductList().containsKey(productKey)){
			getBonusDetails=PremiaService.getInstance().getBonusDetails(policyNo);
				...
				alertMessage(preauthDto, errorMessage,holdMonitorDto);
					...
					try{
						...
						if(isActiveHumanTask){
							...
							preauthDto.setScreenName(SHAConstants.FINANCIALS_APPROVAL_AUTO_ALLOCATION);
							fireViewEvent(MenuPresenter.SHOW_FINANCIAL_APPROVAL_CLAIM_BILLING_SCREEN, preauthDto);
						}else{
	...
	public void alertMessage(final SearchProcessClaimFinancialsTableDTO t, String message,final SearchHoldMonitorScreenTableDTO holdMonitorDto) {
