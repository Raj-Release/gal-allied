...
@Stateless
public class SearchUploadDocumentsSubmitService {

	@PersistenceContext
	protected EntityManager entityManager;
	
	private final Logger log = LoggerFactory.getLogger(SearchUploadDocumentsSubmitService.class); 
		...
		if(null != preauthList && !preauthList.isEmpty())
		{
			for (Preauth preauth : preauthList) {
				if((! preauth.getStatus().getKey().equals(ReferenceTable.CLAIM_REOPENED_STATUS) && (!preauth.getStatus().getKey().equals(ReferenceTable.PREAUTH_APPROVE_STATUS)))
						...
						|| (preauth.getStatus().getKey().equals(ReferenceTable.FINANCIAL_SETTLED)) || (preauth.getStatus().getKey().equals(ReferenceTable.STANDALONE_WITHDRAW_POST_STATUS))){
