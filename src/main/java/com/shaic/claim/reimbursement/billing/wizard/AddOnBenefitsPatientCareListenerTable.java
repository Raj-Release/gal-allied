...
public class AddOnBenefitsPatientCareListenerTable extends ViewComponent {
...
//		addListener();
	...
	void initTable(VerticalLayout layout) {
		...
		table = new Table("Add on Benefits (Patient Care)", data);
		table.removeAllItems();
		table.addStyleName("generateColumnTable");
//		table.setWidth("100%");
		table.setPageLength(table.getItemIds().size());
		
		table.setVisibleColumns(new Object[] { "particulars", "engagedFrom", "engagedTo", "admittedNoOfDays", "allowedNoOfDays", "totalClaimedAmount", "entitledNoOfDays", "noOfDaysPerHospitalization", "entitlementPerDayAmt","utilizedNoOfDays", "balanceAvailable", "eligibleNoofDays", "eligiblePayableNoOfDays", "eligiblePerDayAmt", "eligibleNetAmount", "coPayPercentage", "copayAmount", "netAmountAfterCopay", "limitAsPerPolicy", "payableAmount"  });

		table.setColumnHeader("particulars", "Particulars");
		table.setColumnHeader("engagedFrom", "Engaged From (A)");
		table.setColumnHeader("engagedTo", "Engaged To (B)");
		table.setColumnHeader("admittedNoOfDays", "No. of Days claimed (C)");
...
//		manageListeners();
	...
	public class ImmediateFieldFactory extends DefaultFieldFactory {
		...
		public Field<?> createField(Container container, Object itemId,
			...
			if("allowedNoOfDays".equals(propertyId)) {
			...
			//	field.setConverter(plainIntegerConverter);
				...
				tableRow.put("allowedNoOfDays", field);
				return field;
			}
			else if("totalClaimedAmount".equals(propertyId)) {
					TextField field = new TextField();
					field.setWidth("100px");
					field.setData(benefitsDTO);
					field.setConverter(plainIntegerConverter);
