...
@Stateless
public class UploadDocumentsForAckNotReceivedSubmitService {
	
	@PersistenceContext
	protected EntityManager entityManager;
	
	private final Logger log = LoggerFactory.getLogger(UploadDocumentsForAckNotReceivedSubmitService.class); 
	...
	if(null != reimbList && !reimbList.isEmpty())
	{
		...
		for (Reimbursement reimbursement : reimbList) {
			...
			if(null != docAck)
			  ...
			  //IMSSUPPOR-30937
			  if(reimbursement!=null && reimbursement.getClaim().getIntimation()!=null && reimbursement.getClaim().getIntimation().getProcessClaimType().equalsIgnoreCase(SHAConstants.PA_TYPE)){
				  benefitcover = getCoverValueForViewBasedOnrodKey(reimbursement.getDocAcknowLedgement().getRodKey());
			  }
			  
			  if(null != benefitcover && null != reimbursement.getBenefitsId() && null != reimbursement.getBenefitsId().getValue())
				{
					benefitAndCover = reimbursement.getBenefitsId().getValue() + ", " + benefitcover;
				}
			  
			  uploadDTO.setCoverCode(benefitAndCover);
				tableDTOList.add(uploadDTO);
