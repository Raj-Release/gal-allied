 ...
 *
 */
public class ViewRRCStatusHistory extends ViewComponent {
	...
	public Component getContent() {
		...
		HorizontalLayout detailsLayout = new HorizontalLayout(
				buildEmployeeDetailsLayout(), buildPolicyDetailsLayout(),
				buildHospitalDetailsLayout(), buildInsuredDetailsLayout());
		...
		VerticalLayout verticalRRCLayout = new VerticalLayout(hLayout,
				detailsPanel, buildReductionDetailsPanel(),
				buildRRCReviewDetails(), buildButtonLayout());
		// verticalRRCLayout.setComponentAlignment(hLayout,
	...
	private HorizontalLayout buildRequestFields() {
		txtRRCRequestNo = (TextField) binder.buildAndBind("RRC Request No",
				"rrcRequestNo", TextField.class);
		txtRequestIntiatedDate = (TextField) binder.buildAndBind(
				"Request Initiated Date", "rrcintiatedDate", TextField.class);
	...
	private Panel buildReductionDetailsPanel() {
		txtClaimType = (TextField) binder.buildAndBind("Claim Type",
				"claimType", TextField.class);
		txtProcessingStage = (TextField) binder.buildAndBind(
				"Processing Stage", "processingStage", TextField.class);
		txtClaimType.setReadOnly(true);
		txtProcessingStage.setReadOnly(true);
		/*
		 * cmbSignificantClinicalInformation =
		 * (ComboBox)binder.buildAndBind("Significant Clinincal Information"
		 * ,"significantClinicalInformation",ComboBox.class);
		 */
		txtRemarks = (TextArea) binder.buildAndBind("Remark", "remarks",
				TextArea.class);
		txtRemarks.setRequired(true);

		FormLayout reductionDetails = new FormLayout(txtClaimType,
				txtProcessingStage);
		...
		/*
		 * FormLayout clinicalInfo = new
		 * FormLayout(cmbSignificantClinicalInformation);
		 */
		...
		VerticalLayout vReductionLayout = new VerticalLayout(reductionDetails,
				quantumReductionListenerTable, infoAndRemarksLayout);
		vReductionLayout.setWidth("100%");
		vReductionLayout.setComponentAlignment(reductionDetails,
				Alignment.TOP_RIGHT);
	...
	private Panel buildRRCReviewDetails() {
		txtProcessEligibility = (TextField) binder.buildAndBind("Eligibility",
				"reviewEligiblityValue", TextField.class);
		txtProcessEligibility.setReadOnly(true);

		if (this.bean.getReviewEligiblityValue() == null
				&& this.bean.getEligibility() != null) {
			txtProcessEligibility.setReadOnly(false);
			txtProcessEligibility.setValue(this.bean.getEligibility()
					.getValue());
			txtProcessEligibility.setReadOnly(true);
		}

		rrcRequestProcessedDate = (TextField) binder.buildAndBind(
				"Request Processed Date", "rrcRequestProcessedDate",
				TextField.class);
		rrcRequestProcessedDate.setStyleName(ValoTheme.TEXTFIELD_BORDERLESS);
		rrcRequestProcessedDate.setReadOnly(true);

		extraEffortEmployeeListenerTable = extraEffortEmployeeListenerTableObj
				.get();
		...
		// extraEffortEmployeeListenerTable.setReadOnly(true);

		extraEffortEmployeeListenerTable.setEnabled(false);

		FormLayout rrcProcessingForm = new FormLayout(txtProcessEligibility,
				rrcRequestProcessedDate, extraEffortEmployeeListenerTable);
		...
		rrcProcessingPanel.setCaption("RRC Reviewed Details");
	...
	private void setTableValues() {
		 ...
		 * List<ExtraEmployeeEffortDTO> extraEmployeeList =
		 * this.bean.getEmployeeEffortList(); // List<ExtraEmployeeEffortDTO>
		...
		if (null != extraEffortEmployeeListenerTable) {
			List<ExtraEmployeeEffortDTO> extraEmployeeList = this.bean
					.getEmployeeEffortList();
			// List<ExtraEmployeeEffortDTO> extraEmployeeList =
			// this.bean.getRrcDTO().getEmployeeEffortList();
			if (null != extraEmployeeList && !extraEmployeeList.isEmpty()) {
				Long sNo = 1l;
				for (ExtraEmployeeEffortDTO extraEmployeeEffortDTO : extraEmployeeList) {
					extraEmployeeEffortDTO.setSlNo(sNo);
					BeanItemContainer<SelectValue> empNameContainer = (BeanItemContainer<SelectValue>) containerMap
							.get(SHAConstants.EMPLOYEE_NAME_LIST);
					for (int i = 0; i < empNameContainer.size(); i++) {
						/*
						 * if(null !=
						 * extraEmployeeEffortDTO.getSelEmployeeName()) { if
						 * ((extraEmployeeEffortDTO
						 * .getSelEmployeeName().getValue
						 * ()).equalsIgnoreCase(empNameContainer
						 * .getIdByIndex(i).getValue())) {
						 * extraEmployeeEffortDTO
						 * .setSelEmployeeName(empNameContainer
						 * .getIdByIndex(i)); } }
						 */
						if (null != extraEmployeeEffortDTO.getSelEmployeeId()) {
							if ((extraEmployeeEffortDTO.getSelEmployeeId()
									.getId()).equals(empNameContainer
									.getIdByIndex(i).getId())) {
								extraEmployeeEffortDTO
										.setSelEmployeeName(empNameContainer
												.getIdByIndex(i));
							}
					...
					// extraEffortEmployeeListenerTable.setTableList(extraEmployeeList);
					extraEffortEmployeeListenerTable
							.addBeanToList(extraEmployeeEffortDTO);
					sNo++;
				}
		...
		if (null != quantumReductionListenerTable) {
			QuantumReductionDetailsDTO quantumReductionDetailsDTO = this.bean
					.getQuantumReductionDetailsDTO();
			if (null != quantumReductionDetailsDTO) {
				quantumReductionListenerTable
						.addBeanToList(quantumReductionDetailsDTO);
			}
			/*
			 * List<QuantumReductionDetailsDTO> quantumReductionDetailsList =
			 * this.bean.getQuantumReductionDetailsDTOList(); if(null !=
			 * quantumReductionDetailsList &&
			 * !quantumReductionDetailsList.isEmpty()) {
			 * quantumReductionListenerTable
			 * .setTableList(quantumReductionDetailsList); }
			 */
		}
	...
	public void setTableValuesToDTO() {
		if (null != extraEffortEmployeeListenerTable) {
			List<ExtraEmployeeEffortDTO> extraEmployeeList = extraEffortEmployeeListenerTable
					.getValues();
			if (null != extraEmployeeList && !extraEmployeeList.isEmpty()) {
				Long sNo = 1l;
				for (ExtraEmployeeEffortDTO extraEmployeeEffortDTO : extraEmployeeList) {
	...
	private void loadContainerDataSources() {
		significantClinicalInformation = (BeanItemContainer<SelectValue>) containerMap
				.get(SHAConstants.SIGNIFICANT_CLINICAL_INFORMATION);
		// CR2019204
		/*
		 * cmbSignificantClinicalInformation.setContainerDataSource(
		 * significantClinicalInformation);
		 * cmbSignificantClinicalInformation.setItemCaptionMode
		...
		for (int i = 0; i < significantClinicalInformation.size(); i++) {
			if (this.bean.getSignificantClinicalInformationValue() != null
					&& (this.bean.getSignificantClinicalInformationValue())
							.equalsIgnoreCase(significantClinicalInformation
									.getIdByIndex(i).getValue())) {
				this.cmbSignificantClinicalInformation
						.setValue(significantClinicalInformation
								.getIdByIndex(i));
			}
		}
	...
	private Panel buildEmployeeDetailsLayout() {
		txtEmployeeName = (TextField) binder.buildAndBind("Employee Name",
				"employeeName", TextField.class);
		txtEmployeeId = (TextField) binder.buildAndBind("Employee ID",
				"employeeId", TextField.class);
		txtEmployeeZone = (TextField) binder.buildAndBind("Employee Zone",
				"employeeZone", TextField.class);
		txtEmployeeDept = (TextField) binder.buildAndBind("Employee Dept",
				"employeeDept", TextField.class);
		employeeDetailsPanel = new Panel();
		FormLayout empDetails = new FormLayout(txtEmployeeName, txtEmployeeId,
				txtEmployeeZone, txtEmployeeDept);
	...
	private Panel buildPolicyDetailsLayout() {
		txtPolicyNo = (TextField) binder.buildAndBind("Policy No", "policyNo",
				TextField.class);
		txtIntimationNo = (TextField) binder.buildAndBind("Intimation No",
				"intimationNo", TextField.class);
		txtProductName = (TextField) binder.buildAndBind("Product Name",
				"productName", TextField.class);
		txtDuration = (TextField) binder.buildAndBind("Duration", "duration",
				TextField.class);
		txtSumInsured = (TextField) binder.buildAndBind("Sum Insured",
				"sumInsured", TextField.class);

		policyDetailsPanel = new Panel();
		FormLayout policyDetails = new FormLayout(txtPolicyNo, txtIntimationNo,
				txtProductName, /* txtDuration, */txtSumInsured);
	...
	private Panel buildHospitalDetailsLayout() {
		txtHospitalName = (TextField) binder.buildAndBind("Hospital Name",
				"hospitalName", TextField.class);
		txtHospitalCity = (TextField) binder.buildAndBind("Hospital City",
				"hospitalCity", TextField.class);
		txtHospitalZone = (TextField) binder.buildAndBind("Hospital Zone",
				"hospitalZone", TextField.class);
		txtDateOfAdmission = (TextField) binder.buildAndBind(
				"Date Of Admission", "dateOfAdmission", TextField.class);
		txtDateOfDischarge = (TextField) binder.buildAndBind(
				"Date Of Discharge", "dateOfDischarge", TextField.class);

		hospitalDetailsPanel = new Panel();
		FormLayout hospitalDetails = new FormLayout(txtHospitalName,
				txtHospitalCity, txtHospitalZone, txtDateOfAdmission,
				txtDateOfDischarge);
	...
	private Panel buildInsuredDetailsLayout() {
		txtInsuredName = (TextField) binder.buildAndBind("Insured Name",
				"insuredName", TextField.class);
		txtInsuredAge = (TextField) binder.buildAndBind("Age", "insuredAge",
				TextField.class);
		txtSex = (TextField) binder.buildAndBind("Sex", "sex", TextField.class);

		insuredDetailsPanel = new Panel();
		FormLayout insuredDetails = new FormLayout(txtInsuredName,
				txtInsuredAge, txtSex);
	...
	public void loadRRCRequestDropDownValues(
		...
		// reconsiderationRequest = (BeanItemContainer<SelectValue>)
		// referenceDataMap.get("commonValues");
		cmbSignificantClinicalInformation
				.setContainerDataSource(mastersValueContainer);
		cmbSignificantClinicalInformation
	...
	public void loadEmployeeMasterData(
		...
		extraEffortEmployeeListenerTable.loadEmployeeMasterData(field,
				employeeDetailsList);
	}
