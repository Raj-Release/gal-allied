 ...
 *
 */
public class SearchRRCRequestViewImpl extends AbstractMVPView implements SearchRRCRequestView {
	
	@Inject
	private SearchRRCRequestForm  searchForm;
	
	@Inject
	private SearchRRCRequestTable searchResultTable;
//	private RRCRequestTableForExcelReport searchResultTable;
	
	@Inject
	private RRCRequestTableForExcelReport tableForExcel;
		...
		btnGenerateExcel.addClickListener(new ClickListener() {
			...
			private static final long serialVersionUID = 1L;
				...
				public void buttonClick(ClickEvent event) {
					...
					List<SearchRRCRequestTableDTO> requestTableList = new ArrayList<SearchRRCRequestTableDTO>();
					...
					{						
					    ...
					    requestTableList = dbCalculationService.getRRCCompleteList(intimationNo, rrcRequestNo,
								cpcode, rrcRequestTypeId, rrcEligbilityTypeId, sqlFromDate, sqlToDate);
					...
					{
					for (SearchRRCRequestTableDTO searchRRCRequestTableDTO2 : requestTableList) {
						
						if(("true").equalsIgnoreCase(searchRRCRequestTableDTO2.getCheckBoxStatus()))
							chkBoxValue = true;
					}
					...
					if(chkBoxValue || (null != chkBox && chkBox.getValue()))
					{
						...
						excelExport.setReportTitle("RRC History");
						excelExport.export();
					}
					else
					{
	...
	private void getTableDataForReport(List<SearchRRCRequestTableDTO> finalList) {
		if (null != tableForExcel) {
			tableForExcel.removeRow();

			List<SearchRRCRequestTableDTO> requestTableList = finalList;
			if (null != requestTableList && !requestTableList.isEmpty()) {
				/*
	...
	public void doSearch() {
		SearchRRCRequestFormDTO searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		fireViewEvent(SearchRRCRequestPresenter.SEARCH_BUTTON_CLICK_RRC_REQUEST, searchDTO,userName,passWord);
	...
	public void list(Page<SearchRRCRequestTableDTO> tableRows) {
		...
		else
		{
			...
			Button homeButton = new Button("Search RRC Request Home");
