...
//	private FormLayout legaHeirLayout;
	...
	private FormLayout thirdFLayout;
	
	private FormLayout fourthFLayout;
	...
	private FormLayout typeOfAdmissionFLayout;
...
//				bean.setIsPedWatchList(false);
	...
	public Component getContent() {
			...
			if(detailsTableDTOs !=null && !detailsTableDTOs.isEmpty()){
				for(DiagnosisDetailsTableDTO detailsTableDTO:detailsTableDTOs){
					if(detailsTableDTO != null && detailsTableDTO.getSublimitName() != null && detailsTableDTO.getSublimitName().getName() != null
						...
						showInfoMessageBox("Waiting period of 12 months from the policy inception is applicable<br>"+"and payable once in a lifetime");
		...
		if(bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideFlag() != null && 
				bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideFlag().equals(SHAConstants.YES_FLAG)){
			//SHAUtils.showMessageBoxWithCaption("Excluded Provider Hospital", "Information");

			String exclusionProvideRemarks = bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideRemarks();
			if(exclusionProvideRemarks!=null){
				SHAUtils.showMessageBoxWithCaption("Excluded Provider Hospital<br>"+exclusionProvideRemarks, "Information");
			}else{
				SHAUtils.showMessageBoxWithCaption("Excluded Provider Hospital", "Information");
			}
		}
...
//		} 
		...
		if(bean.getNewIntimationDTO().getIntimationId() != null) {
			...
			String result = dbCalculationService.getHospitalClaimAlertFlag(intimationNumber);
      ...
      //code added for CR glx2020167 by noufel
		...
		/**
		 * for GLX2020099 'No of days' rename as 'Length of Stay'
		 */
		...
		//ventilatorSupportOption.setValue(this.bean.getPreauthDataExtractionDetails().getVentilatorSupport()? true : false);
...
//		changeInDiagnosis = new OptionGroup("Change In Diagnosis");
		...
		firstFLayout = new FormLayout(admissionDate, reasonForChangeInDOA,
				cmbRoomCategory,ventilatorSupportOption, illnessFLayout, cmbSpecifyIllness, dischargeDate, systemOfMedicineTxt,domicillaryHospitalisation,cmbHospitalCashDueTo,cmbHospitalisationDueTo);
	...
	//	firstFLayout.setMargin(true);
		...
		thirdFLayout = new FormLayout(optPatientDayCare,cmbPatientDayCareValue);
		/**
		 * GLX2020132
		 */
		
		
		chkEmergency=new CheckBox("Emergency           <b style= 'color: red'> *</b>");
		chkEmergency.setCaptionAsHtml(true);
		List<Reimbursement> reimbursementDetails = reimbursementService.getRembursementDetails(this.bean.getClaimKey());
		if(reimbursementDetails != null && !reimbursementDetails.isEmpty()){
		  for (Reimbursement reimbursement : reimbursementDetails) {
			 ...
			 chkEmergency.setRequired(true);
			 this.bean.getPreauthDataExtractionDetails().setEmergency(true);
			 break;
		    }
		  }
		}
		if(this.bean.getNewIntimationDTO().getHospitalDto() != null && this.bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideFlag() != null
				&& this.bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideFlag().equals(SHAConstants.YES_FLAG)){
				chkEmergency.setEnabled(true);
				chkEmergency.setRequired(true);
			}else{
				chkEmergency.setEnabled(false);
				chkEmergency.setValue(null);
				chkEmergency.setRequired(false);
		}
		
		
		
		fourthFLayout=new FormLayout(chkEmergency);
		...
		HorizontalLayout formHLayout = new HorizontalLayout(firstFLayout,
				secondFLayout,fourthFLayout);
		
		if(this.bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
				this.bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode().equalsIgnoreCase(SHAConstants.PRODUCT_CODE_076)
				|| (this.bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
				this.bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode().equalsIgnoreCase(SHAConstants.GROUP_HOSPITAL_CASH_POLICY))){
			 formHLayout = new HorizontalLayout(firstFLayout,
					secondFLayout,thirdFLayout,fourthFLayout);
		}else{
			cmbHospitalCashDueTo.setVisible(false);
			cmbHospitalCashDueTo.setEnabled(false);
			firstFLayout.removeComponent(cmbHospitalCashDueTo);
		}
...
//			dischargeDate.setEnabled(false);
		...
		cmbTypeOfAdmission = (ComboBox) binder.buildAndBind("Type Of Admission","admissionType", ComboBox.class);
		cmbTypeOfAdmission.setEnabled(false);
		typeOfAdmissionFLayout = new FormLayout(cmbTypeOfAdmission);
		typeOfAdmissionFLayout.setMargin(true);
		
		
				
		/*As per satish sir instruction change in diagnosis layout removed i.e fLayout*/
		wholeVLayout = new VerticalLayout(billclasihori,formHLayout, /*fLayout,*/ sectionDetailsListenerTableObj, diagnosisDetailsTableInstance, typeOfAdmissionFLayout,dynamicElementsHLayout,treatingTableLayout,implantHLayout,implantTableVLayout, tableVLayout, fLayout1, otherClaimsFLayout, preandPostHospitalLayout);
...
//			wholeVLayout = new VerticalLayout(formHLayout, fLayout, sectionDetailsListenerTableObj, diagnosisDetailsTableInstance, dynamicElementsHLayout, tableVLayout, fLayout1, otherClaimsFLayout, preandPostHospitalLayout, hospitalDetailsObj, preauthCoordinatorViewInstance);
		...
		if(null != this.bean.getPreauthDataExtractionDetails().getEmergency()){
			chkEmergency.setValue(this.bean.getPreauthDataExtractionDetails().getEmergency()); 
	    }
...
//		Below Code for SCRC REVISED - CR20181302
		...
		if (this.bean.getPreauthDataExtractionDetails().getVentilatorSupport()!= null) {
			System.out.println(String.format("Ventilator Support value in set up references: [%s]", this.bean.getPreauthDataExtractionDetails().getVentilatorSupport()));
...
//		this.claimedDetailsTableObj.setDBCalculationValues((Map<String, Double>)referenceData.get("claimDBDetails"));
		...
		if (bean.getPreauthDataExtractionDetails().getAdmissionType() != null) {
			cmbTypeOfAdmission.setValue(bean.getPreauthDataExtractionDetails().getAdmissionType());
		}
...
//		}
		...
		if(this.bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
			...
			if(optPatientDayCare != null && optPatientDayCare.getValue() != null && optPatientDayCare.getValue().toString() == "true"){
		...
		List<DiagnosisDetailsTableDTO> diagnosisValues =  diagnosisDetailsTableObj.getValues();
		for (DiagnosisDetailsTableDTO diagnosisDetailsTableDTO : diagnosisValues) {
			if(diagnosisDetailsTableDTO.getIcdCode() != null && diagnosisDetailsTableDTO.getIcdCode().getId() != null && diagnosisDetailsTableDTO.getIcdCode().getId().equals(SHAConstants.COVID_19_ICD_IDENT_KEY)){
				if(cmbTypeOfAdmission !=null && cmbTypeOfAdmission.getValue() == null){
					hasError = true;
...
//					this.bean.setIsDishonoured(true);
				...
				if(this.bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
					...
					if ((null != bean.getReceiptOfDocumentsDTO().getDocumentDetails()
						...
						fireViewEvent(ClaimRequestDataExtractionPagePresenter.SAVE_MA_HOSPITAL_CASH_PHC_TABLE_VALUES, this.bean.getReceiptOfDocumentsDTO());
					}
					...
					{
						fireViewEvent(ClaimRequestDataExtractionPagePresenter.SAVE_MA_HOSPITAL_CASH_TABLE_VALUES, this.bean.getReceiptOfDocumentsDTO());
					}
				}
		 ...
		 cmbSection.addValueChangeListener(new ValueChangeListener() {
	  			...
	  			public void valueChange(ValueChangeEvent event) {
	  				...
	  				if(cmbSection != null && cmbSection.getValue() != null){
		  				...
		  				if(null != bean.getNewIntimationDTO().getPolicy().getProduct().getKey() &&
		  						(ReferenceTable.STAR_CARDIAC_CARE_PLATIANUM.equals(bean.getNewIntimationDTO().getPolicy().getProduct().getKey()))){
		  					...
		  					if(null != bean.getPreauthDataExtractionDetails().getSection().getValue() &&
		  						...
		  						if(null != bean.getNewIntimationDTO().getPolicy().getPolicyType().getKey() &&
		  							...
		  							SHAUtils.showAlertMessageBox(SHAConstants.CARDIAC_CARE_PLATINUM_SECTION_ALERT);   						
		  						}
...
//									event.getProperty().setValue(null);
							 ...
							 if(bean.getNewIntimationDTO().getPolicy().getProduct().getKey().equals(ReferenceTable.STAR_GRP_COVID_PROD_KEY_LUMSUM)){
								 ...
								 Long totalAlertDte = null;
							    	if(bean.getPreauthDataExtractionDetails().getAdmissionDate() != null
							    			&& bean.getPreauthDataExtractionDetails().getDischargeDate() != null){
							    		Date DOA = bean.getPreauthDataExtractionDetails().getAdmissionDate();
							    		Date changeDT = (Date) event.getProperty().getValue();
								    	
								    	alertDte = SHAUtils.getDiffDays(DOA, changeDT);
						...
						if(bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
							...
							Date enteredDate = (Date) ((DateField) event.getProperty()).getValue();
							bean.getReceiptOfDocumentsDTO().getDocumentDetails().setDateOfDischarge(enteredDate);
							setTableValuesForHospCash();
						}
...
//							dischargeDate.setValue(null);
					...
					if (!SHAUtils.isDateOfAdmissionWithPolicyRange(policyFromDate,policyToDate,enteredDate)) {
						 isValid = false;
					...
					if(bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
							...
							|| (bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode() != null && 
							bean.getClaimDTO().getNewIntimationDto().getPolicy().getProduct().getCode().equalsIgnoreCase(SHAConstants.GROUP_HOSPITAL_CASH_POLICY))){
						bean.getReceiptOfDocumentsDTO().getDocumentDetails().setDateOfAdmission(enteredDate);
						setTableValuesForHospCash();
					}
				}
...
//				}
		...
		cmbTypeOfAdmission.addValueChangeListener(new Property.ValueChangeListener() {
			private static final long serialVersionUID = -8435623803385270083L;
			...
			public void valueChange(ValueChangeEvent event) {
				SelectValue value = (SelectValue) event.getProperty().getValue();
				if(value != null){
					bean.getPreauthDataExtractionDetails().setAdmissionType(value);
				}
				fireViewEvent(ClaimRequestWizardPresenter.CLAIM_REQUEST_TYPE_OF_ADMISSION_CHANGED,null);
			}
		});
		...
		.addValueChangeListener(new Property.ValueChangeListener() {
			...
			@Override
			public void valueChange(ValueChangeEvent event) {
				Boolean checkValue = chkEmergency.getValue();
				
				if(checkValue != null && checkValue.equals(Boolean.TRUE)){
                    if(bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideFlag() != null && 
					bean.getNewIntimationDTO().getHospitalDto().getExclusionProvideFlag().equals(SHAConstants.YES_FLAG)){
                    	bean.getPreauthDataExtractionDetails().setEmergency(checkValue);
                    } else {
                    	bean.getPreauthDataExtractionDetails().setEmergency(Boolean.FALSE);
                    }
				}else{
					bean.getPreauthDataExtractionDetails().setEmergency(checkValue);
				}
	...
	public Boolean popupMessageForWaitingPeriod() {
		...
		Date issuedPeriodEndDate = SHAUtils.formatDateWithoutTime("31-03-2021");
 		...
 		if((diffDays != null && diffDays < 30)
 				...
 				&& (ReferenceTable.getCovidProducts().contains(productCode))
 				&& (SHAUtils.isDateOfIntimationWithPolicyRange(issuedPeriodStartDate, issuedPeriodEndDate, policyFromDate))
 				&& (SHAUtils.isDateOfIntimationWithPolicyRange(issuedPeriodStartDate, issuedPeriodEndDate, currnetDate)) && !is15DaysWaitingPeriod){
 			//popupMessageFor30DaysWaitingPeriod();
 			is15DaysWaitingPeriod=false;
 		}
...
//				legalHeirDetails.setIFSCView(viewSearchCriteriaWindow);
	...
	public void generateadmissiontypeFields(Boolean displayadmissiontype) {
		if (displayadmissiontype) {
			cmbTypeOfAdmission.setRequired(true);
			cmbTypeOfAdmission.setEnabled(true);
			BeanItemContainer<SelectValue> typeOdAdmission1 = (BeanItemContainer<SelectValue>) referenceData.get("typeofAdmission");
			cmbTypeOfAdmission.setContainerDataSource(typeOdAdmission1);
			cmbTypeOfAdmission.setItemCaptionMode(ItemCaptionMode.PROPERTY);
			cmbTypeOfAdmission.setItemCaptionPropertyId("value");

			if (this.bean.getPreauthDataExtractionDetails().getAdmissionType() != null) {
				this.cmbTypeOfAdmission.setValue(this.bean
						.getPreauthDataExtractionDetails().getAdmissionType());
			}
			...
			List<DiagnosisDetailsTableDTO> diagnosisList = this.diagnosisDetailsTableObj.getValues();
			for (DiagnosisDetailsTableDTO diagnosisDetailsTableDTO : diagnosisList) {
				if(diagnosisDetailsTableDTO != null && diagnosisDetailsTableDTO.getIcdCode() != null &&  diagnosisDetailsTableDTO.getIcdCode().getId() != null && diagnosisDetailsTableDTO.getIcdCode().getId().equals(SHAConstants.COVID_19_ICD_IDENT_KEY)){
					duplicateCovidCount = duplicateCovidCount+1;
				}
			}
			if(duplicateCovidCount == 0){
				cmbTypeOfAdmission.setRequired(false);
				cmbTypeOfAdmission.setEnabled(false);
				cmbTypeOfAdmission.clear();
				this.bean.getPreauthDataExtractionDetails().setIsHomeIsolationExist(false);				
			}
		}
	...
	public void genertateFieldsBasedOnTypeOfAdmisstion() {
		...
		if (cmbTypeOfAdmission !=null && cmbTypeOfAdmission.getValue() != null 
				&& cmbTypeOfAdmission.getValue().toString() !=null && this.bean.getClaimDTO().getClaimType() != null &&
				this.bean.getClaimDTO().getClaimType().getId().equals(ReferenceTable.CASHLESS_CLAIM_TYPE_KEY)) {
			if (this.cmbTypeOfAdmission.getValue().toString().toLowerCase()
					.contains(SHAConstants.HOME_ISOLATION)) {
				this.bean.getPreauthDataExtractionDetails().setIsHomeIsolationExist(true);
			}else{
				this.bean.getPreauthDataExtractionDetails().setIsHomeIsolationExist(false);
			}
		}
	 ...
	 private void setTableValuesForHospCash()
		...
		public void setBenefitsData(List<AddOnBenefitsDTO> benefitsDTO) {
			...
			Double totalApprovedAmt = 0d;
			for (AddOnBenefitsDTO addOnBenefitsDTO : benefitsDTO) {
