 ...
 *
 */
public class SearchGeneratePARemainderBulkForm extends SearchComponent<SearchGenerateRemainderBulkFormDTO> {
	
	@Inject
	private SearchGeneratePARemainderBulkTable searchTable;
	
	private List<BulkReminderResultDto> prevBatchList = new ArrayList<BulkReminderResultDto>();
	
	@Inject
	private PABulkReminderListExpoTable searchresultExportTable;
	        ...
	        public void selectedTabChange(SelectedTabChangeEvent event) {
	        	...
	        	if(tabSheet != null && ! tabSheet.isEmpty()){
	        		if((tabSheet.toLowerCase()).equalsIgnoreCase(SHAConstants.PRINT_TAB_BULK_REMINDER)){
	        			...
	        			fireViewEvent(SearchGeneratePARemainderBulkPresenter.GET_PA_PREV_BATCH_LIST,null);
...
//				String searchFilter = (String)event.getButton().getData();				
				if(searchfields != null && !searchfields.isEmpty()){
					fireViewEvent(SearchGeneratePARemainderBulkPresenter.SEARCH_PA_BATCH_ID_BUTTON_CLICK, searchfields);
				}
				else{
...
//		return printLetterTab;
	...
	public SearchGeneratePARemainderBulkTable getSearchTable(){
	...
	public SearchGenerateRemainderBulkFormDTO getSearchFilters(){
		...
		try {			
				...
				if(binder.isValid()){
					...
					errorMsg += "Please Provide Atleast one Value for Search Condtion<br>";
					hasError = true;
				}
...
//				}				
				...
				if(!hasError){
					...
					bean.setLobId(ReferenceTable.PA_LOB_KEY);
				}
				else{
