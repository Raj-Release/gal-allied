...
@Stateless
public class PASearchSettlementPullBackService extends AbstractDAO<Intimation> {
	
	@Inject
	private AcknowledgementDocumentsReceivedService ackDocReceivedService;
	Map<Long, Object> workFlowMap= new WeakHashMap<Long, Object>();
	
	public PASearchSettlementPullBackService(){
		
	}
	
	
	public  Page<PASearchSettlementPullBackDTO> searchForSettlementPullBack(
			PASearchSettlementPullBackFormDTO formDTO,
			String userName, String passWord) {
		
		
		Page<PASearchSettlementPullBackDTO> page = new Page<PASearchSettlementPullBackDTO>();
		...
		List<PASearchSettlementPullBackDTO> dtoList = new ArrayList<PASearchSettlementPullBackDTO>();
						...
						try {
							...
							for (Reimbursement reimbursement : rodByClaimKey) {
								...
								if(faApprovedAndNotLotCreated) {
									...
									if(null != workflowKey) {
										PASearchSettlementPullBackDTO filledDTO = getFilledDTO(intimationByNo, claimforIntimation, reimbursement, userName, passWord, true);
...
//						dtoList.add(getFilledDTO(intimationByNo, claimforIntimation, null, userName, passWord, false));
	...
	protected PASearchSettlementPullBackDTO getFilledDTO(Intimation intimation, Claim claim, Reimbursement reimbursement,  String userName, String password, Boolean isAllowedToPullBack) {
		PASearchSettlementPullBackDTO dto = new PASearchSettlementPullBackDTO();
...
//		dto.setPolicyDivCode(intimation.getPolicy().get);
		if(reimbursement != null) {
			...
			dto.setApprovedAmount(ackDocReceivedService.getBenefitAddOnOptionalApprovedAmt(reimbursement).longValue());
...
//				}
		...
		Double insuredSumInsured = dbCalculationService.getInsuredSumInsured(intimation.getInsured().getInsuredId().toString(), intimation.getPolicy().getKey()
				,intimation.getInsured().getLopFlag());
