...
public class PAHospSettlementPullBackUI extends ViewComponent {
	...
	protected void showOrHideValidation(Boolean isVisible, Component clearingRemarks) {
	 ...
	 private Panel buildRegistrationPanel() {
			...
			String caption = "Claim Final Approval Cancel";
	...
	public void initView(PAHospSearchSettlementPullBackDTO dto, NewIntimationDto newIntimationDTO) {
...
//		viewTrailsBtn.addStyleName(ValoTheme.BUTTON_LINK);
	...
	private HorizontalLayout buildSubmitAndCancelBtnLayout(final PAHospSearchSettlementPullBackDTO dto) {
									...
									"No", "Yes", new ConfirmDialog.Listener() {
										...
										public void onClose(ConfirmDialog dialog) {
											if (!dialog.isConfirmed()) {
												fireViewEvent(PAHospSettlementPullBackPresenter.SUBMIT_SETTLEMENT_PULL_BACK, dto);
											} else {
					...
					ConfirmDialog dialog = ConfirmDialog
							.show(getUI(),
									...
									"No", "Yes", new ConfirmDialog.Listener() {
										...
										public void onClose(ConfirmDialog dialog) {
											if (!dialog.isConfirmed()) {
												fireViewEvent(MenuItemBean.PA_HOSP_SETTLEMENT_PULL_BACK,
														null);
											} else {
	...
	private void buildSuccessfulLayout() {
		...
		finalOKButton.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				popup.close();
				fireViewEvent(MenuItemBean.PA_CLEAR_CASHLESS,
						null);
