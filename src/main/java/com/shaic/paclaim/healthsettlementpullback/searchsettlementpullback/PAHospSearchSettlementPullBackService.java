...
@Stateless
public class PAHospSearchSettlementPullBackService extends AbstractDAO<Intimation> {
	...
	public PAHospSearchSettlementPullBackService(){
		
	}
	
	
	public  Page<PAHospSearchSettlementPullBackDTO> searchForSettlementPullBack(
			PAHospSearchSettlementPullBackFormDTO formDTO,
			String userName, String passWord) {
		Page<PAHospSearchSettlementPullBackDTO> page = new Page<PAHospSearchSettlementPullBackDTO>();
		...
		List<PAHospSearchSettlementPullBackDTO> dtoList = new ArrayList<PAHospSearchSettlementPullBackDTO>();
						...
						try {
							...
							for (Reimbursement reimbursement : rodByClaimKey) {
								...
								if(faApprovedAndNotLotCreated) {
									...
									if(null != workflowKey) {
										PAHospSearchSettlementPullBackDTO filledDTO = getFilledDTO(intimationByNo, claimforIntimation, reimbursement, userName, passWord, true);
...
//						dtoList.add(getFilledDTO(intimationByNo, claimforIntimation, null, userName, passWord, false));
	...
	protected PAHospSearchSettlementPullBackDTO getFilledDTO(Intimation intimation, Claim claim, Reimbursement reimbursement,  String userName, String password, Boolean isAllowedToPullBack) {
		PAHospSearchSettlementPullBackDTO dto = new PAHospSearchSettlementPullBackDTO();
...
//				}
		...
		if(intimation.getInsured() != null && intimation.getPolicy() != null){
			...
			if(null != intimation && null != intimation.getPolicy() && 
				...
				insuredSumInsured = dbCalculationService.getInsuredSumInsured(intimation.getInsured().getInsuredId().toString(), intimation.getPolicy().getKey(),intimation.getInsured().getLopFlag());
			}
			else
			{
				insuredSumInsured = dbCalculationService.getGPAInsuredSumInsured(intimation.getInsured().getInsuredId().toString(), intimation.getPolicy().getKey());
			}
