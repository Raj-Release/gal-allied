...
public class PAPreauthEnhancementPreviousClaimDetailsPage extends PreMedicalPreauthPreviousClaimsPageUI implements WizardStep<PreauthDTO> {
		...
		viewType.addValueChangeListener(new Property.ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if (viewType.getValue().equals(POLICY_WISE)) {						
					...
					fireViewEvent(PAPreauthEnhancemetWizardPresenter.PREAUTH_PREVIOUS_CLAIM_DETAILS, bean);
		...
		cmbRelapseOfIllness.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				if(event.getProperty() != null && (SelectValue)event.getProperty().getValue() != null ){//&& ((SelectValue)event.getProperty().getValue()).getId() == ReferenceTable.COMMONMASTER_YES) {
					fireViewEvent(PAPreauthEnhancemetWizardPresenter.PREAUTH_RELAPSE_OF_ILLNESS_CHANGED, bean, (SelectValue)event.getProperty().getValue());
				}
