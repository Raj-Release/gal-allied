...
import com.shaic.paclaim.cashless.downsize.wizard.PADownSizePreauthPdfPage;
...
public class PADownsizePreauthRequestWizardViewImpl extends AbstractMVPView implements PADownsizePreauthRequestWizard, GWizardListener {
	...
	@Inject
	private Instance<PADownsizePreauthRequestDataExtractionPage> downSizePreauthDataExtractionPage;
	
	@Inject
	private Instance<PADownSizePreauthPdfPage> downSizePreauthPdfPage;
	
	private PADownSizePreauthPdfPage downSizePreauthPdfObj;
	
	@Inject
	private Instance<PAClaimedAmountPageForDownsizeRequest> claimedAmountDetailsPageInstance;
	
	private PAClaimedAmountPageForDownsizeRequest claimedAmountDetailsPageObj;

	@Inject
	private Instance<RevisedCashlessCarousel> commonCarouselInstance;
	
	private PADownsizePreauthRequestDataExtractionPage downsizePreauthPageObj;
...
//		preauthIntimation.init(bean.getNewIntimationDTO(), bean.getClaimDTO(),"Downsize Pre-auth");
		...
		PADownsizePreauthRequestDataExtractionPage downsizePreauthPageObj=downSizePreauthDataExtractionPage.get();
		downsizePreauthPageObj.init(this.bean,this.wizard);
		this.downsizePreauthPageObj=downsizePreauthPageObj;
		
		PADownSizePreauthPdfPage downSizePreauthPdfObj=downSizePreauthPdfPage.get();
...
//>>>>>>> cb48332ef255a08cffcca1164ace30af4466a6cb
	...
	{
		fireViewEvent(PADownsizePreauthRequestWizardPresenter.VALIDATE_DOWNSIZE_PREAUTH_REQUEST_USER_RRC_REQUEST, bean);//, secondaryParameters);
	}
	...
	public void buildValidationUserRRCRequestLayout(Boolean isValid) {
		...
		else
		{
			...
			rewardRecognitionRequestViewObj.initPresenter(SHAConstants.PA_PROCESS_DOWNSIZE_REQUEST_PREAUTH);
	...
	public void activeStepChanged(WizardStepActivationEvent event) {
		fireViewEvent(PADownsizePreauthRequestWizardPresenter.DOWNSIZE_STEP_CHANGE_EVENT, event);
	...
	public void wizardCompleted(WizardCompletedEvent event) {
		...
		fireViewEvent(PADownsizePreauthRequestWizardPresenter.DOWNSIZE_SUBMIT_EVENT, this.bean);
