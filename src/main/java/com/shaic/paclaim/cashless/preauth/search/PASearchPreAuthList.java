...
public class PASearchPreAuthList extends GBaseTable<PASearchPreauthTableDTO> {
	
	private final Logger log = LoggerFactory.getLogger(PASearchPreAuthList.class);
...
//							SHAUtils.setDBActiveOrDeactiveClaim(wrkFlowKey,session)	;
							fireViewEvent(PAMenuPresenter.SHOW_PREATUH_WIZARD, t);
						}else{
				...
				alertMessage(t, errorMessage);
					...
					try{
						if(isActiveHumanTask){
							...
							{
								fireViewEvent(PAMenuPresenter.SHOW_FLP_BYPASS_PREAUTH_WIZARD, t);
							}
