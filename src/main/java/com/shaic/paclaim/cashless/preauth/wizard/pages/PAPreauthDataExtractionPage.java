...
public class PAPreauthDataExtractionPage extends ViewComponent implements WizardStep<PreauthDTO> {

	private static final long serialVersionUID = -3471303898495849899L;
	...
	//	age.setEnabled(false);
		...
		wholeVLayout = new VerticalLayout(formHLayout,sectionDetailsListenerTableObj, diagnosisDetailsTableInstance, dynamicElementsHLayout, tableVLayout, lLayout);
		...
		if(null != bean && null != bean.getNewIntimationDTO() && null != bean.getNewIntimationDTO().getPolicy() &&
			...
			wholeVLayout = new VerticalLayout(formHLayout,gpaLayout,sectionDetailsListenerTableObj, diagnosisDetailsTableInstance, dynamicElementsHLayout, tableVLayout, lLayout);
		}
...
//		popup.setContent( viewDocumentDetailsPage);
	...
	public void setupReferences(Map<String, Object> referenceData) {
		...
		fireViewEvent(PAPreauthWizardPresenter.SET_DB_DETAILS_TO_REFERENCE_DATA, this.bean);
...
//		}
		...
		*/
		...
		if (this.procedurdTableObj != null) {
			boolean isValid = this.procedurdTableObj.isValidPA();
			if (!isValid) {
				hasError = true;
		...
		criticalIllnessChk
				.addValueChangeListener(new Property.ValueChangeListener() {
					...
					public void valueChange(ValueChangeEvent event) {
						...
						fireViewEvent(
								PAPreauthWizardPresenter.CHECK_CRITICAL_ILLNESS,
								checkValue, true);
					}
				});
		...
		cmbTreatmentType.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(value != null && cmbTreatmentType.getValue().toString().toLowerCase()
					...
					if(!bean.getPreauthMedicalProcessingDetails().getProcedureExclusionCheckTableList().isEmpty()) {
						...
						fireViewEvent(
								PAPreauthWizardPresenter.PREAUTH_TREATMENT_TYPE_CHANGED,
								null);
					}
				} else {
					fireViewEvent(
		  ...
		  cmbSection.addValueChangeListener(new ValueChangeListener() {
   			...
   			public void valueChange(ValueChangeEvent event) {
   				...
   				fireViewEvent(PAPreauthWizardPresenter.SUBLIMIT_CHANGED_BY_SECTION, bean);
...
//					}
		...
		cmbPatientStatus.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				fireViewEvent(
						PAPreauthWizardPresenter.PREAUTH_PATIENT_STATUS_CHANGED,
						null);
			}
		});
...
//				if(value != null) {
					fireViewEvent(
							PAPreauthWizardPresenter.PREAUTH_HOSPITALISATION_DUE_TO,
							value);
//				}
		...
		this.sectionDetailsListenerTableObj.dummySubCoverField.addValueChangeListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				...
				if(value != null ) {
					fireViewEvent(PAPreauthWizardPresenter.SET_DB_DETAILS_TO_REFERENCE_DATA, bean);
	    ...
	    public void generatedFieldsBasedOnHospitalisationDueTo(SelectValue value) {
			if (null != value) {
				if(value.getId() != null && ReferenceTable.INJURY_MASTER_ID.equals(value.getId())) {
					...
					reportedToPolice.addValueChangeListener(new Property.ValueChangeListener() {
						...
						public void valueChange(ValueChangeEvent event) {
							...
							if(event.getProperty() != null && event.getProperty().getValue() != null) {
								fireViewEvent(PAPreauthWizardPresenter.PRE_AUTH_REPORTED_TO_POLICE, isChecked);
							}
						}
					});
					...
					firstFLayout.addComponent(NotAdheringFLayout);
