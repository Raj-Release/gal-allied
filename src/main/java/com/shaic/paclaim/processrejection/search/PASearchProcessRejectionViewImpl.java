...
public class PASearchProcessRejectionViewImpl  extends AbstractMVPView implements PASearchProcessRejectionView , Searchable  {
	

	private static final long serialVersionUID = 1934939436987293748L;

	@Inject
	private PASearchProcessRejectionForm searchForm;
	
	@Inject
	private PASearchProcessRejectionTable searchResultTable;
		...
		else
		{
			...
			homeButton.addClickListener(new ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					dialog.close();
					fireViewEvent(MenuItemBean.PA_PROCESS_PREAUTH_REJECTION, null);
	...
	public void doSearch() {
		...
		fireViewEvent(PASearchProcessRejectionPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof PASearchProcessRejectionTable)
			{
				((PASearchProcessRejectionTable) comp).removeRow();
			}
		}
	}
