...
public class PAHealthReopenClaimPageUI extends ViewComponent {
	...
	private PAHealthReopenClaimPageDTO bean;
	
	@Inject
	private PAHealthReopenClaimTable pAHealthReopenClaimTable;
...
//	private ReopenClaimReimbursementTable reopenClaimReimbursementTableObj;
	...
	@Inject
	private Instance<PAHealthReopenReimbursementEditTable> reopenClaimReimbursementTableInstance;
	
	private PAHealthReopenReimbursementEditTable reopenClaimReimbursementTableObj;
	...
	public void init(PAHealthReopenClaimPageDTO bean){
		
		this.bean = bean;
		
		pAHealthReopenClaimTable.init("Re-Open Claim Details", false, false);
		
		List<PAHealthReopenClaimTableDTO> reopenClaimList = bean.getReopenClaimList();
		pAHealthReopenClaimTable.setTableList(reopenClaimList);
		...
		mainLayout = new VerticalLayout(pAHealthReopenClaimTable,this.previousPreAuthDetailsTableObj,this.reopenClaimReimbursementTableObj,reasonsLayout,buttonHorLayout);
	...
	public void addListener(){
				        ...
				        "No", "Yes", new ConfirmDialog.Listener() {
				            ...
				            public void onClose(ConfirmDialog dialog) {
				                if (!dialog.isConfirmed()) {
				                	fireViewEvent(MenuItemBean.PA_HOSP_RE_OPEN_CLAIM_CLAIM_LEVEL, true);
				                } else {
				                    dialog.close();
				                }
				            }
		...
		btnSubmit.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PAHealthReopenClaimPresenter.SUBMIT_REOPEN_CLAIM, bean);
				}
