 ...
 *
 */
public class PASearchProcessClaimFinancialsNonHospViewImpl extends AbstractMVPView implements PASearchProcessClaimFinancialsNonHospView{

	
	@Inject
	private PASearchProcessClaimFinancialsNonHospForm  searchForm;
	
	@Inject
	private PASearchProcessClaimFinancialsNonHospTable searchResultTable;
	...
	public void doSearch() {
		...
		if(claimedAmountFrom != null && claimedAmountTo != null)  {
			if((claimedAmountFrom > claimedAmountTo || claimedAmountTo < claimedAmountFrom)) {
				 getErrorMessage("Claimed Amount From should be less than Claimed Amount To");
			}else{
				fireViewEvent(PASearchProcessClaimFinancialsNonHospPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
			}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof PASearchProcessClaimFinancialsNonHospTable)
			{
				((PASearchProcessClaimFinancialsNonHospTable) comp).removeRow();
			}
		}
