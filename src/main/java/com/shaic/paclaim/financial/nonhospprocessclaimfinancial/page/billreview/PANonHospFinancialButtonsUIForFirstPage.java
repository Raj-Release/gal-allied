...
import com.shaic.domain.BonusWaiver;
	...
	private void addListener() {
		...
		approveBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(list != null){
					...
					if(tableList != null){
						for (PABalanceSumInsuredTableDTO paBalanceSumInsuredTableDTO : tableList) {
							...
							cummulativeBonus += paBalanceSumInsuredTableDTO.getCumulativeBonus();
						}
					}
				}
				...
				String insertBonusSts = PremiaService.getInstance().getInsertBonusDetailsResult(intimationDtls.getPolicy().getPolicyNumber());
					if(insertBonusSts != null && insertBonusSts.equalsIgnoreCase("Success")){
					...
					BonusResponse bonusResponse = bonusApi.getBonusDetails(bean.getPolicyDto().getPolicyNumber(), insuredDtls.getHealthCardNumber(),  bean.getPolicyDto().getProduct().getCode());
						if(bonusResponse != null){
							if(bonusResponse.getStatus() != null){
								...
								if(bonusResponse != null && bonusResponse.getCurrentYearBonus() != null){
									Double toolBonus = Double.valueOf(bonusResponse.getCurrentYearBonus());
									if(cummulativeBonus != null && toolBonus != null && cummulativeBonus > toolBonus){
										bonusStatus = "FAIL";
									}
								}
							}	
				...
				BonusWaiver bonusWaiver = claimService.getBonusWaiver(intimationDtls.getIntimationId());
				if(validatePolicyStatus(bean.getPolicyDto().getPolicyNumber()) || (intimationDtls.getAllowApprovalFlag() != null && intimationDtls.getAllowApprovalFlag().equalsIgnoreCase("Y"))){
					...
					if((bonusStatus != null && bonusStatus.equalsIgnoreCase("PASS"))
							|| (bonusWaiver !=null && bonusWaiver.getActiveFlag().equalsIgnoreCase("1") && isSIExhaust)){	
						isApproval = true;
...
public void disableOnlyPaymentQuery(Boolean isEnabled){
	paymentQuery.setEnabled(isEnabled);
}
}
