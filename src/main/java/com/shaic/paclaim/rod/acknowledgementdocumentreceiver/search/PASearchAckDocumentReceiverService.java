...
@Stateless
public class PASearchAckDocumentReceiverService extends AbstractDAO<Intimation>{

	
	@PersistenceContext
	protected EntityManager entityManager;
	
	public PASearchAckDocumentReceiverService() {
		super();
	}
	...
	public  Page<SearchAcknowledgementDocumentReceiverTableDTO> search(
		...
		if(accidentOrDeath != null){
			Predicate condition9 = criteriaBuilder.like(root.<String>get("incidenceFlag"),incidenceType);
	...
	//	Predicate condition8 = criteriaBuilder.equal(root.<Long>get("lobId"), ReferenceTable.PA_LOB_KEY);
	    ...
	    Predicate condition13 = criteriaBuilder.equal(root.<Intimation>get("intimation").<Long>get("lobId"), ReferenceTable.PA_LOB_KEY);
	    Predicate condition12 = criteriaBuilder.or(condition11,condition8,condition13);
		...
		/*Predicate condition5 = criteriaBuilder.like(root.<Intimation>get("intimation").<String>get("processClaimType"), "%"+SHAConstants.PA_TYPE+"%");
...
//		}
		...
		for (SearchAcknowledgementDocumentReceiverTableDTO searchAcknowledgementDocumentReceiverTableDTO : tableDTO) {
			if(searchAcknowledgementDocumentReceiverTableDTO.getCpuCode() != null){
				...
				if(null != searchAcknowledgementDocumentReceiverTableDTO.getIncidenceFlag())
				{
				if(searchAcknowledgementDocumentReceiverTableDTO.getIncidenceFlag().equalsIgnoreCase(SHAConstants.ACCIDENT_FLAG))
				{
					searchAcknowledgementDocumentReceiverTableDTO.setAccidentOrDeath(SHAConstants.ACCIDENT);
				}
				else
				{
					searchAcknowledgementDocumentReceiverTableDTO.setAccidentOrDeath(SHAConstants.DEATH);
				}
				}
				
			}
			if(searchAcknowledgementDocumentReceiverTableDTO.getProductKey() != null && ReferenceTable.getGPAProducts().containsKey(searchAcknowledgementDocumentReceiverTableDTO.getProductKey())){
				if(searchAcknowledgementDocumentReceiverTableDTO.getPaPatientName() != null){
					searchAcknowledgementDocumentReceiverTableDTO.setInsuredPatientName(searchAcknowledgementDocumentReceiverTableDTO.getPaPatientName());
				}
			}
		}
