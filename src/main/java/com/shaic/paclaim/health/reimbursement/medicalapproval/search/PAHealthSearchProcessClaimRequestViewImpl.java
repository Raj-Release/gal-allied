 ...
 *
 */
public class PAHealthSearchProcessClaimRequestViewImpl extends AbstractMVPView implements PAHealthSearchProcessClaimRequestView{

	
	@Inject
	private PAHealthSearchProcessClaimRequestForm  searchForm;
	
	@Inject
	private PAHealthSearchProcessClaimRequestTable searchResultTable;
	...
	public void initViewForReferesh(PAHealthSearchProcessClaimRequestFormDTO dto, Boolean shouldDoSearch){
	...
	public void doSearch() {
		PAHealthSearchProcessClaimRequestFormDTO searchDTO = searchForm.getSearchDTO();
		...
		if(claimedAmountFrom != null && claimedAmountTo != null)  {
			if((claimedAmountFrom > claimedAmountTo || claimedAmountTo < claimedAmountFrom)) {
				 ...
				 fireViewEvent(PAHealthSearchProcessClaimRequestPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord,screenName);
			}
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof PAHealthSearchProcessClaimRequestTable)
			{
				((PAHealthSearchProcessClaimRequestTable) comp).removeRow();
			}
		}
		...
		else
		{
			...
			if(searchDTO.getIntimationNo() != null && !searchDTO.getIntimationNo().isEmpty() && searchDTO.getIntimationNo().length() > 0  && 
				...
				if(null != screenName && (screenName.equalsIgnoreCase(SHAConstants.PA_HEALTH_MEDICAL_WAIT_FOR_INPUT_SCREEN))){
					homeButton.setCaption("Wait for Input - Hospitalisation Home");
				}
