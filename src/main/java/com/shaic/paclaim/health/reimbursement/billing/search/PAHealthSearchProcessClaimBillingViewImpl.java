 ...
 *
 */
public class PAHealthSearchProcessClaimBillingViewImpl extends AbstractMVPView implements PAHealthSearchProcessClaimBillingView{

	
	@Inject
	private PAHealthSearchProcessClaimBillingForm  searchForm;
	
	@Inject
	private PAHealthSearchProcessClaimBillingTable searchResultTable;
	...
	public void doSearch() {
		PAHealthSearchProcessClaimBillingFormDTO searchDTO = searchForm.getSearchDTO();
		Pageable pageable = searchResultTable.getPageable();
		searchDTO.setPageable(pageable);
		String userName=(String)getUI().getSession().getAttribute(BPMClientContext.USERID);
		String passWord=(String)getUI().getSession().getAttribute(BPMClientContext.PASSWORD);
		fireViewEvent(PAHealthSearchProcessClaimBillingPresenter.SEARCH_BUTTON_CLICK, searchDTO,userName,passWord);
	...
	public void resetSearchResultTableValues() {
		...
		while(componentIter.hasNext())
		{
			...
			if(comp instanceof PAHealthSearchProcessClaimBillingTable)
			{
				((PAHealthSearchProcessClaimBillingTable) comp).removeRow();
			}
		}
	...
	public void init(BeanItemContainer<SelectValue> cpucode,
			BeanItemContainer<SelectValue> productNameCode, BeanItemContainer<SelectValue> typeContainer, BeanItemContainer<SelectValue> selectValueForPriority, BeanItemContainer<SelectValue> statusByStage) {
		searchForm.setCBXValue(cpucode, productNameCode,typeContainer,selectValueForPriority,statusByStage);
