 ...
 *
 */
public class PAHealthSearchProcessClaimRequestForm extends SearchComponent<PAHealthSearchProcessClaimRequestFormDTO> {
	
	@Inject
	private PAHealthSearchProcessClaimRequestTable searchTable;
	...
	private PAHealthSearchProcessClaimRequestFormDTO dto =  new PAHealthSearchProcessClaimRequestFormDTO();
	...
	public void initView(PAHealthSearchProcessClaimRequestFormDTO dto, Boolean shouldDoSearch) {
		initBinder(dto);
...
//		formLayoutNext.setMargin(true);
		...
		FormLayout formLayout4 = new FormLayout(txtClaimedAmountFrom,txtClaimedAmountTo,cbxType);
	...
	private void initBinder(PAHealthSearchProcessClaimRequestFormDTO dto)
	{
		if(dto != null) {
			this.dto = dto;
		} else {
			this.dto = new PAHealthSearchProcessClaimRequestFormDTO();
		}
		
		this.binder = new BeanFieldGroup<PAHealthSearchProcessClaimRequestFormDTO>(PAHealthSearchProcessClaimRequestFormDTO.class);
	...
	public void setCBXValue(BeanItemContainer<SelectValue> intimationSource,
		...
		if(null != screenName && (SHAConstants.PA_HEALTH_MEDICAL_SCREEN.equalsIgnoreCase(screenName))){
			mainPanelName = "PA Health Process Claim Request";
		}
		else if(null != screenName && (SHAConstants.PA_HEALTH_MEDICAL_WAIT_FOR_INPUT_SCREEN.equalsIgnoreCase(screenName)))
		{
			mainPanelName = "Wait for Input - Hospitalisation";
		}
			...
			public void valueChange(Property.ValueChangeEvent event) {
				...
				if(cbxTreatmentType.getValue() != null ){
					...
					if(ReferenceTable.MEDICAL.equals(cbxTreatmentType.getValue().toString())){
						...
						fireViewEvent(PAHealthSearchProcessClaimRequestPresenter.SPECIALITY, ReferenceTable.MEDICAL_CODE );
						...
						fireViewEvent(PAHealthSearchProcessClaimRequestPresenter.SPECIALITY,ReferenceTable.SURGICAL_CODE );
