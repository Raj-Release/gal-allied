...
public class PAHealthFinancialProcessPageViewImpl extends AbstractMVPView implements PAHealthFinancialProcessPageWizard {

	private static final long serialVersionUID = -1756934701433733987L;
	
	@Inject
	private PAHealthFinancialProcessPageUI billingProcessPage;
	...
	public Component getContent() {
		...
		fireViewEvent(PAHealthFinancialProcessPagePresenter.BILLING_PROCESS_SET_UP_REFERENCE, bean);
		return comp;
	}
		...
		if(billingProcessPage.validatePage()) {
			try {
				if(bean.getPostHospitalizaionFlag() || bean.getPreHospitalizaionFlag()) {
					...
					fireViewEvent(PAHealthFinancialProcessPagePresenter.POST_HOSPITALIZATION_AMOUNT, bean);
				}
