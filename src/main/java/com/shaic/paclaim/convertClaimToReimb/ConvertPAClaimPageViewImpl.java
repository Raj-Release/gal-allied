...
public class ConvertPAClaimPageViewImpl extends AbstractMVPView implements ConvertPAClaimPageView {
	...
	@Inject
	private Instance<ConvertPAClaimPage> ConvertClaimPageInstance;
	...
	private ConvertPAClaimPage convertClaimPage;
	...
	public void init(ConvertClaimDTO bean,BeanItemContainer<SelectValue> selectValueContainer,SearchConvertClaimTableDto searchFormDto)
	{
		this.bean=bean;
		...
		this.intimationDto=bean.getClaimDTO().getNewIntimationDto();
...
//		spaceVeritcLayout.setHeight("100px");
	...
	{
		fireViewEvent(ConvertPAClaimPagePresenter.VALIDATE_CONVERT_PA_CLAIM_USER_RRC_REQUEST, preauthDTO);//, secondaryParameters);
	}
	...
	public void buildValidationUserRRCRequestLayout(Boolean isValid) {
		...
		else
		{
			...
			rewardRecognitionRequestViewObj.initPresenter(SHAConstants.PA_PROCESS_CONVERT_CLAIM);
	...
	public void buildRRCRequestSuccessLayout(String rrcRequestNo) {
		...
		if(null == rewardRecognitionRequestViewObj)
		{
		rewardRecognitionRequestViewInstance.get().buildRRCRequestSuccessLayout(rrcRequestNo);
		}
		else
		{
