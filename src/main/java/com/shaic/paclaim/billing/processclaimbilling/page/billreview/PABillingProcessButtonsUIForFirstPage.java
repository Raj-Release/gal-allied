...
public class PABillingProcessButtonsUIForFirstPage extends ViewComponent  {
	...
	public void initView(PreauthDTO bean, GWizard wizard) 
	{
		...
		HorizontalLayout buttonFirstLayout = new HorizontalLayout(referCoordinatorBtn,referToMedicalApproverBtn, cancelROD,referToBillEntry,sendToClaimApprovalBtn,sendToFA);
		
		if(null != referToBillEntry)
			referToBillEntry.setEnabled(true);
		
		if(null != sendToFA)
			sendToFA.setEnabled(false);
		...
		if(null != bean.getBenefitId() && 
				((ReferenceTable.DEATH_BENEFIT_MASTER_VALUE).equals(bean.getBenefitId()) || (ReferenceTable.PTD_BENEFIT_MASTER_VALUE).equals(bean.getBenefitId())))
		{		
			sendToFA.setEnabled(false);
			sendToClaimApprovalBtn.setEnabled(true);
		}
		else
		{
			sendToClaimApprovalBtn.setEnabled(false);
			sendToFA.setEnabled(true);
		}
		
		if(null != bean.getIsPaymentQuery() && (SHAConstants.YES_FLAG).equalsIgnoreCase(bean.getIsPaymentQuery()) &&
...
//		}
	...
	private void addListener() {
		...
		referCoordinatorBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PABillingReviewPagePresenter.BILLING_REFERCOORDINATOR_EVENT, null);
			}
		});
		...
		referToMedicalApproverBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				wizard.getNextButton().setEnabled(Boolean.FALSE);
				wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(PABillingReviewPagePresenter.BILLING_REFER_TO_MEDICAL_APPROVER_EVENT, null);
			}
		});
		
		
		sendToClaimApprovalBtn = new Button("Send to Claim Approval");		
		sendToClaimApprovalBtn.setHeight("-1px");
		//Vaadin8-setImmediate() sendToClaimApprovalBtn.setImmediate(true);
		sendToClaimApprovalBtn.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(PABillingReviewPagePresenter.BILLING_SEND_TO_CLAIM_APPROVAL_EVENT, null);
			}
		});
		
		sendToFA = new Button("Send to Financial Approval");		
		sendToFA.setHeight("-1px");
		//Vaadin8-setImmediate() sendToFA.setImmediate(true);
		sendToFA.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				wizard.getNextButton().setEnabled(Boolean.TRUE);
				wizard.getFinishButton().setEnabled(Boolean.FALSE);
				fireViewEvent(PABillingReviewPagePresenter.BILLING_SEND_TO_FA_EVENT, null);
			}
		});
		...
		cancelROD.addClickListener(new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				wizard.getNextButton().setEnabled(Boolean.FALSE);
				wizard.getFinishButton().setEnabled(Boolean.TRUE);
				fireViewEvent(PABillingReviewPagePresenter.BILLING_CANCEL_ROD_EVENT, null);
			}
		});
		...
		btnBillAssessmentSheet.addClickListener(new Button.ClickListener() {
		 ...
		 referToBillEntry.addClickListener(new Button.ClickListener() {
				...
				public void buttonClick(ClickEvent event) {
					wizard.getNextButton().setEnabled(Boolean.FALSE);
					wizard.getFinishButton().setEnabled(Boolean.TRUE);
					fireViewEvent(PABillingReviewPagePresenter.BILLING_REFER_TO_BILL_ENTRY, null);
				}
			});
	...
	public void buildSendToClaimApprovalLayout()
	{
		...
		VerticalLayout VLayout = new VerticalLayout(new FormLayout(billingRemarks,reasonForClaimApprovalRemarks), btnLayout);
		VLayout.setWidth("800px");
		VLayout.setMargin(true);
		showInPopup(VLayout, dialog);
...
//		});
	...
	public boolean isValid()
	{
		...
		if(this.binder == null) {
			hasError = true;
			errorMessages.add("Please select Cancel ROD or Refer to coordinator or Refer to Medical Approver or Send to Claim Approval. </br>");
			return !hasError;
		}
