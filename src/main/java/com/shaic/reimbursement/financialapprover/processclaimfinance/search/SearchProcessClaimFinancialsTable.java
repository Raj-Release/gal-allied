 ...
 *
 */
public class SearchProcessClaimFinancialsTable extends GBaseTable<SearchProcessClaimFinancialsTableDTO>{
			...
			public void buttonClick(ClickEvent event) {
					...
					try{
						if( isActiveHumanTask){
							...
							t.setScreenName(SHAConstants.PROCESS_CLAIM_FINANCIALS);
			...
			if(t.getProductKey() != null && ! ReferenceTable.getGMCProductList().containsKey(t.getProductKey())){
				 ...
				 if(getBonusDetails != null){
						intimationService.updateCumulativeBonusFromWebService(getBonusDetails);
				       	if(getBonusDetails.getChequeStatus() != null  && ! getBonusDetails.getChequeStatus().isEmpty()){
   ...
   private Boolean checkPCCRequestavalible(String intimationNO){
	   ...
	   if(pccRequest != null && pccRequest.getStatus() !=null 
		   ...
		   if(!pccRequest.getStatus().getKey().equals(SHAConstants.PCC_COORDINATOR_APPROVED_STATUS)){
			   getErrorMessage("PCC response yet to be received for this intimation");
			   return false;
		   }   
	   }
	   return true;
