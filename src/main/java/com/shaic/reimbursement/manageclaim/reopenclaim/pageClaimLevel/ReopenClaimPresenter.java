...
@ViewInterface(ReopenClaimView.class)
public class ReopenClaimPresenter extends AbstractMVPPresenter<ReopenClaimView>{
	
	public static final String SUBMIT_REOPEN_CLAIM = "submit reopen claim";
	...
	protected void submitCloseClaim(
		...
		ReopenClaimPageDTO bean = (ReopenClaimPageDTO)parameters.getPrimaryParameter();
...
//		payloadType.setIntimation(intimationType);
		System.out.println("---Start time of GET TASH REOPEN-CLAIM presenter---"+System.currentTimeMillis()+" "+bean.getIntimationNumber());
		...
		System.out.println("---End time of GET TASH REOPEN-CLAIM presenter---"+System.currentTimeMillis()+" "+bean.getIntimationNumber());
...
//		 dbCalculationService.reopenClaimProcedure(claim.getIntimation().getIntimationId());
...
//					dbCalculationService.reimbursementRollBackProc(hospitalizationRod.getKey(),"R");
			...
			createRodService.updateReimbursementReopenClaim(bean.getUserName(), reimbursement);
			 ...
			 if(reimbursement.getStage().getKey().equals(ReferenceTable.CLAIM_REQUEST_STAGE)
				 ...
				 System.out.println("---Start time of ACCU REOPEN-CLAIM presenter---"+System.currentTimeMillis()+" "+bean.getIntimationNumber());
				 dbCalculationService.invokeReimbursementAccumulatorProcedure(reimbursement.getKey());
				 System.out.println("---End time of ACCU REOPEN-CLAIM presenter---"+System.currentTimeMillis()+" "+bean.getIntimationNumber());
		    	}
			 System.out.println("---Start time of PROVISION REOPEN-CLAIM presenter---"+System.currentTimeMillis()+" "+bean.getIntimationNumber());
			dbCalculationService.updateProvisionAmount(reimbursement.getKey(), reimbursement.getClaim().getKey());
			System.out.println("---End time of PROVISION REOPEN-CLAIM presenter---"+System.currentTimeMillis()+" "+bean.getIntimationNumber());
			
		}
		
		createRodService.submitReopenClaim(bean);
		
		for (Map<String, Object> map : dbTaskForCloseClaim) {
			
			Long wrkFlowKey = (Long) map.get(SHAConstants.WK_KEY);
			String IntimationNo = (String)map.get(SHAConstants.INTIMATION_NO);
			System.out.println("---Start time of SEC_REOPEN_CLAIM presenter---"+System.currentTimeMillis()+" "+IntimationNo);
		   	dbCalculationService.submitProcedureReopenClaim(wrkFlowKey, IntimationNo,"claimshead");
		   	System.out.println("---End time of SEC_REOPEN_CLAIM presenter---"+System.currentTimeMillis()+" "+IntimationNo);
