...
public class ReOpenRodLevelClaimPage extends ViewComponent {
	...
	@Inject
	private ReOpenRodLevelClaimTable reOpenClaimTable;
	
	private TextArea txtReOpenRemarks;
	
	private BeanFieldGroup<ReOpenRodLevelClaimDTO> binder;
	
	private ReOpenRodLevelClaimDTO bean;
	...
	private SearchReOpenClaimRodLevelTableDTO searchDTO;
	...
	public void init(SearchReOpenClaimRodLevelTableDTO searchDTO){
		...
		fireViewEvent(ReOpenRodLevelClaimPresenter.SET_DATA_FIELD, searchDTO);
		
		this.binder = new BeanFieldGroup<ReOpenRodLevelClaimDTO>(ReOpenRodLevelClaimDTO.class);
		this.binder.setItemDataSource(this.bean);
		binder.setFieldFactory(new EnhancedFieldGroupFieldFactory());
		
		reOpenClaimTable.init("Document Details", false, false);
		
		fireViewEvent(ReOpenRodLevelClaimPresenter.SET_TABLE_DATA, searchDTO);
...
public void addListener(){
				        ...
				        "No", "Yes", new ConfirmDialog.Listener() {
				            ...
				            public void onClose(ConfirmDialog dialog) {
				                if (!dialog.isConfirmed()) {
				                	fireViewEvent(MenuItemBean.RE_OPEN_CLAIM_ROD_LEVEL, true);
				                } else {
				                    dialog.close();
				                }
				            }
		...
		submitBtn.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if(validatePage()){
					fireViewEvent(ReOpenRodLevelClaimPresenter.SUBMIT_DATA, searchDTO,bean);
				}
	...
	public void setUpReference(ReOpenRodLevelClaimDTO bean, BeanItemContainer<SelectValue> reasonForReOpen){
		this.bean = bean;
	...
	public boolean validatePage() {
		...
		if(reOpenClaimTable != null){
			...
			if(isValid){
				eMsg.append("Please Select atlease one ROD for reOpen Claim");
				hasError = true;
			}
		}
