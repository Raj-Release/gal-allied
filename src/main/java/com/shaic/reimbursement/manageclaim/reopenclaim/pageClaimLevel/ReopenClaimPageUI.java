...
public class ReopenClaimPageUI extends ViewComponent {
	...
	private ReopenClaimPageDTO bean;
	
	@Inject
	private ReopenClaimTable reopenClaimTable;
...
//	private ReopenClaimReimbursementTable reopenClaimReimbursementTableObj;
	...
	@Inject
	private Instance<ReopenReimbursementEditTable> reopenClaimReimbursementTableInstance;
	
	private ReopenReimbursementEditTable reopenClaimReimbursementTableObj;
	...
	public void init(ReopenClaimPageDTO bean){
		
		this.bean = bean;
		
		reopenClaimTable.init("Re-Open Claim Details", false, false);
		
		List<ReopenClaimTableDTO> reopenClaimList = bean.getReopenClaimList();
		reopenClaimTable.setTableList(reopenClaimList);
		...
		mainLayout = new VerticalLayout(reopenClaimTable,this.previousPreAuthDetailsTableObj,this.reopenClaimReimbursementTableObj,reasonsLayout,buttonHorLayout);
	...
	public void addListener(){
				        ...
				        "No", "Yes", new ConfirmDialog.Listener() {
				            ...
				            public void onClose(ConfirmDialog dialog) {
				                if (!dialog.isConfirmed()) {
				                	fireViewEvent(MenuItemBean.RE_OPEN_CLAIM_CLAIM_LEVEL, true);
				                } else {
				                    dialog.close();
				                }
				            }
		...
		btnSubmit.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				fireViewEvent(ReopenClaimPresenter.SUBMIT_REOPEN_CLAIM, bean);
				}
		...
		public boolean validatePage() {
			...
			if(cmbReasonForReopen != null && cmbReasonForReopen.getValue() == null){
				eMsg.append("Please Choose Reason for Re-Open Claim </br>");
				hasError = true;
			}
			...
			if(txtProvisionAmount != null && ((txtProvisionAmount.getValue() == null) || (txtProvisionAmount.getValue() != null && txtProvisionAmount.getValue().equalsIgnoreCase("")))){
				eMsg.append("Please Enter Provision Amount </br>");
				hasError = true;
			}
			...
			if(txtReopnRemarks != null && ((txtReopnRemarks.getValue() == null) || (txtReopnRemarks.getValue() != null && txtReopnRemarks.getValue().equalsIgnoreCase("")))){
				eMsg.append("Please Enter Re-Open Remarks </br>");
				hasError = true;
			}
			...
			if(this.reopenClaimReimbursementTableObj != null && this.reopenClaimReimbursementTableObj.getValues() != null 
				...
				for (ViewDocumentDetailsDTO rodList : this.reopenClaimReimbursementTableObj.getValues()) {
					...
					if(rodList.getApprovedAmount() == null){
						eMsg.append("Please Enter Provision Amount for All ROD </br>");
						hasError = true;
					}
			...
			if(txtProvisionAmount != null && txtProvisionAmount.getValue() != null){
				...
				if(bean.getBalanceSI() != null && doubleFromStringWithComma != null && doubleFromStringWithComma > bean.getBalanceSI()){
					...
					eMsg.append("Provison Amount should be less than balance Sum Insured. </br>");
					hasError = true;
