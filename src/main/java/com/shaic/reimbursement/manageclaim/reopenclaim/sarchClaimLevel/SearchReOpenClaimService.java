 ...
 */
@Stateless
public class SearchReOpenClaimService  extends AbstractDAO<Claim>{

	@PersistenceContext
	protected EntityManager entityManager;
	
	private final Logger log = LoggerFactory.getLogger(SearchReOpenClaimService.class);
	
	
	public SearchReOpenClaimService() {
		super();
		
	}
	public  Page<SearchReOpenClaimTableDTO> search(
			SearchReOpenClaimFormDTO searchFormDTO,
		...
		try{
			...
			/*Commented below condition and add equal condition for Slowness Issue
			...
			List<SearchReOpenClaimTableDTO> tableDTO = SearchReOpenClaimMapper.getInstance().getClaimDTO(doList);
		
			tableDTO = getHospitalDetails(tableDTO);
			List<SearchReOpenClaimTableDTO> result = new ArrayList<SearchReOpenClaimTableDTO>();
			result.addAll(tableDTO);
			
			if(tableDTO != null && !tableDTO.isEmpty()){
				for (SearchReOpenClaimTableDTO searchReOpenClaimTableDTO : tableDTO) {
				
					Long claimKey = searchReOpenClaimTableDTO.getClaimKey();
					...
					if(closeClaimList != null && ! closeClaimList.isEmpty()){
						...
						searchReOpenClaimTableDTO.setReasonForClose(closeClaimList.get(0).getClosingReasonId() != null ? closeClaimList.get(0).getClosingReasonId().getValue() : "");
					}
			...
			Page<SearchReOpenClaimTableDTO> page = new Page<SearchReOpenClaimTableDTO>();		
	...
	private List<SearchReOpenClaimTableDTO> getHospitalDetails(
			List<SearchReOpenClaimTableDTO> tableDTO) {
...
//	}
	...
	public List<Reimbursement> submitReopenClaim(SearchReOpenClaimTableDTO tableDTO, ReOpenRodLevelClaimDTO bean){
