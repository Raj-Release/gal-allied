 ...
 */
public class SuggestingContainer extends BeanItemContainer<SelectValue> {
...
public SuggestingContainer(MasterService service) throws IllegalArgumentException {
  ...
  private void filterItems(String filterString, String userId) {
	if (!isNewItemAdded())
	{
		...
		if(userId != null && !userId.isEmpty()){
			selectValueList = service.getDoctorToReallocateTo(filterString, userId);
		}else{
			if(filterString.isEmpty() && SHAUtils.dianosisListValue != null && comboBoxValue.isEmpty()){
				selectValueList = SHAUtils.dianosisListValue.getItemIds();
			}else{
				if(comboBoxValue != null && ! comboBoxValue.isEmpty() && filterString.isEmpty()){
					selectValueList = service.getDiagnosisSelctValuesList(comboBoxValue.toUpperCase());
				}else{
					selectValueList = service.getDiagnosisSelctValuesList(filterString.toUpperCase());
				}
