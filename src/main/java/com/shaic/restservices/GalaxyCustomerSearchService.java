...
@Path("/v1")
@Stateless
public class GalaxyCustomerSearchService {

	private final Logger log = LoggerFactory.getLogger(GalaxyCustomerSearchService.class);
	...
	public String searchGalaxyIntimation(GalaxyCustomerSearchRequest searchObj, @HeaderParam("Authorization") String argParam) {
			...
			if(!StringUtils.isBlank(argParam) && argParam.contains("Basic")){
				...
				if(decodedValue.contains(":")){
					...
					if(dbService.validateRequest(searchObj)){
						if(searchObj != null){
							if((StringUtils.isBlank(searchObj.getSearch()) || searchObj.getSearch().equals("null"))){
								JSONArray dataArr = new JSONArray();
								JSONObject errorMsg = new JSONObject();
								errorMsg.put("data", dataArr);
								errorMsg.put("message", "Search parameter is Mandatory");
								...
								responseString = dbService.performCustomerIdSearch(searchObj);
							}
						}
					}else{
						JSONArray dataArr = new JSONArray();
