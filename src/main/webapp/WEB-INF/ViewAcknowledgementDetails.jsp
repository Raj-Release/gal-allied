        		...
        		<div class="col-md-12">
            		<div class="panel panel-default">
                		<div class="panel-heading">
                    		<p class = "text-left"><strong>Acknowledgement Details</strong></p>
               			 </div>
                		<div class="panel-body">
                    		<div class="table-responsive">
								<table class="table table-condensed table-bordered table-hover">
								<thead>
									<tr><td><strong>Acknowledge No</strong></td><td ><strong>Document Received From</strong></td><td ><strong>Document Received Date</strong></td><td ><strong>Mode Of Receipt</strong></td>
									<td ><strong>Bill Classification</strong></td><td></td></tr>
								</thead>
								<tbody>
									<c:forEach items="${docAcknList}" var="ack">
									
									<tr><td>${ack.getAcknowledgeNumber()}</td><td>${ack.getReceivedFromValue()}</td><td>${ack.getDocumentReceivedDate()}</td><td>${ack.getModeOfReceipt()}</td><td>${ack.getBillClassification()}</td>
									<%-- <td><form action = "ClaimStatusServlet" method="post"><a href="claimstatus?int_num=${docAckNo}&ack_num=${ack.getAcknowledgeNumber()}" target="_blank">View Acknowledgement</a></form></td> --%>
									<td><form action = "ClaimStatusServlet" method="post"><a href="claimstatus?acknowledgementDetails=${ack.getAcknowledgementNoToken()}" target="_blank">View Acknowledgement</a></form></td>
									</tr>
					...
					<input type="text" style="border: none;visibility: hidden;" id="getQueryDate" value="${query.getQueryRaisedDateStr()}">
					 <%
        				request.getSession().removeAttribute("intimationNumber");
					%>
