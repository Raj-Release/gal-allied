...
List<String> PccDocURLList;
 ...
 <% 
	...
	PccDocURLList = (List<String>) session.getAttribute("PccDocURLList");
 %>
  ...
  <c:set var="doctorRemark" scope="session" value=""/>
   ...
   <c:forEach items="${param}" var="currentParam">
	...
	if(("PRE_AUTHORIZATION").match(fileType))
	{
	 ...
	 else if (fileType.match("Post Cashless Cell(PCC)"))
	 {
	 	myArray = <%=net.sf.json.JSONSerializer.toJSON(PccDocURLList) %>;
	 	
	 	fileList = <%=net.sf.json.JSONSerializer.toJSON(pccDMSDocFileList) %>;
	 }
	  else 
	 {
